...
- **Brain backend**: `istota_brain_kind` (`claude_code` default, or `native`), `istota_brain_native_*`, `istota_brain_source_type_overrides`, `istota_brain_room_selectable` (which kinds a room may pin for itself, and which a scheduled job may pin in CRON.md; empty by default, so both are off until you name kinds). See [docs/configuration/native-brain.md](../../docs/configuration/native-brain.md) for the native-brain runbook.
...
| SMS integration | `istota_sms_enabled` | `false` |
