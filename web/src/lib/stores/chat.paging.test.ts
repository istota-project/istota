...
function room(id: number, name = `Room ${id}`): ChatRoom {
	return {
		...
		created_at: '', updated_at: '', origin: 'web', unread_count: 0,
	};
}
...
function asstTurn(taskId: number, text: string): ChatHistory['messages'][number] {
	return {
		role: 'assistant', text, task_id: taskId, status: 'completed',
		created_at: '2026-06-10T12:00:01Z', segments: [{ kind: 'text', text }],
	};
}
...
describe('chat store — scroll-up paging', () => {
	...
	it('seeds hasMore / oldestCursor from the first load', async () => {
		api.getRoomMessages.mockResolvedValue({
			...
			active_task: null, active_tasks: [],
			has_more: true, oldest_cursor: { ts: '2026-06-10 12:00:00', id: 5 },
		});
	...
	it('loadOlder prepends an older page and advances the cursor', async () => {
		...
		api.getRoomMessages.mockResolvedValueOnce({
			messages: [userTurn(2, 'q2'), asstTurn(2, 'a2')],
			active_task: null, active_tasks: [],
			has_more: true, oldest_cursor: { ts: '2026-06-10 12:00:00', id: 3 },
		});
		const s = await freshSession();
		await s.init();
		...
		api.getRoomMessages.mockResolvedValueOnce({
			...
			active_task: null, active_tasks: [],
			has_more: false, oldest_cursor: { ts: '2026-06-10 11:00:00', id: 1 },
		});
		await s.loadOlder();
