...
function thinks(m: ChatMessage): Extract<Segment, { kind: 'thinking' }>[] {
  return m.segments.filter(
    (s): s is Extract<Segment, { kind: 'thinking' }> => s.kind === 'thinking',
  );
}
...
describe('renderGroups — body layout', () => {
  ...
  it('consecutive tools (and short narration between them) coalesce into one chip', () => {
    ...
    expect(kinds(groups)).toEqual(['activity', 'prose']);
    const activity = groups[0] as Extract<
      ReturnType<typeof renderGroups>[number],
      { kind: 'activity' }
    >;
    ...
    const m: ChatMessage = {
      ...
      segments: [
        { kind: 'text', id: 's0', text: 'short lead-in', settled: true },
        {
          kind: 'tool',
          id: 'h1',
          tool: { id: 'h1', name: '', description: 'edit', running: false },
        },
        { kind: 'text', id: 's2', text: meaty, settled: true },
        {
          kind: 'tool',
          id: 'h3',
          tool: { id: 'h3', name: '', description: 'edit', running: false },
        },
