<script lang="ts">
  ...
  function buildPingPointsGeoJSON(pings: LocationPing[]): GeoJSON.FeatureCollection {
    const sorted = [...pings].sort((a, b) => Date.parse(a.timestamp) - Date.parse(b.timestamp));
    ...
    map.addLayer({
      ...
      paint: {
        'circle-radius': [
          ...
          ['zoom'],
          8,
          ['/', ['get', 'radius_meters'], mPerPxBase / 256], // z8
          16,
          ['/', ['get', 'radius_meters'], mPerPxBase / 65536], // z16
          22,
          ['/', ['get', 'radius_meters'], mPerPxBase / 4194304], // z22
        ],
    ...
    map.addLayer({
      ...
      paint: {
        'circle-radius': [
          ...
          ['zoom'],
          8,
          ['/', ['get', 'radius_meters'], mPerPxBase / 256],
          16,
          ['/', ['get', 'radius_meters'], mPerPxBase / 65536],
          22,
          ['/', ['get', 'radius_meters'], mPerPxBase / 4194304],
        ],
  ...
  function updateSources() {
    ...
    if (pingSrc)
      pingSrc.setData(buildPingPointsGeoJSON(filterAccuratePings(pings, activeActivityTypes)));
...
<style>
  ...
  @keyframes pulse {
    0% {
      box-shadow: 0 0 0 0 rgba(74, 255, 127, 0.5);
    }
    100% {
      box-shadow: 0 0 0 12px rgba(74, 255, 127, 0);
    }
  }
