...
| `Select`            | Every dropdown                           | bits-ui backed. `size="md"` when it shares a row with a default `Button`; `sm` (the default) for a dense toolbar. `fullWidth` for settings forms so it matches text inputs                                                                                                                                                                                                                                                                                                                |
...
- Controls that share a row share a height: `--control-height-{sm,md}`, applied as `min-height`. `Button`, `Chip` and the `Select` trigger each used to compute their own box from a font size, a padding and (for `Select`) a border, and landed on four different answers — a `Select` beside a `Button` sat ~2.6px short, by an amount that _changed with the text-scale setting_, since everything but the border is in `rem`. Set both on any new inline control. `min-height` rather than `height`, so a wrapped label still sets the box.
...
**Money list page**: the record-table shell is defined once as globals in `routes/money/+layout.svelte` — `.money-toolbar`, `.money-notice-bar`, `.money-table`, `.money-table-header`, `.money-table-row` (`--tree` for the denser accounts and report trees), `.money-sortable`, `.money-status`, `.money-amount`, `.money-kebab-spacer`, `.money-table-empty`, `.money-control-input`, `.money-result-count`, `.money-sort-arrow`. A page styles only its own columns (widths, alignment) and inherits everything else. The shell fixes the inline edge at `0.75rem` for every element on the page; do not set your own. The three report pages share a second shell, `.report-frame` in `routes/money/reports/+layout.svelte` (`.section-header`, `.section-toggle`) — deliberately not named `.money-*`, since `stray-money-global` reserves that prefix for the layout above. They had each declared `:global(.section-toggle)` byte-identically, and **a `:global()` in a route file leaks app-wide**, so which copy won came down to stylesheet order.
