...
const mockChatRooms: MockChatRoom[] = [
	{ id: 1, token: 'web-alice-general', name: 'general', archived: false, created_at: new Date().toISOString(), updated_at: new Date().toISOString(), model: 'claude-opus-4-8', effort: 'high' },
];
...
(() => {
	...
	for (let i = 0; i < SEED_COUNT; i++) {
		...
		mockChatTasks.set(id, {
			id, roomToken: 'web-alice-general',
...
(() => {
	...
	mockChatTasks.set(200, { id: 200, roomToken: 'web-alice-general', prompt: 'tighten the moving-plan note', createdAt: base, variant: 'multiround' });
	mockChatTasks.set(201, { id: 201, roomToken: 'web-alice-general', prompt: 'fix the near-expiry 401s in auth', createdAt: base + 12_000, variant: 'multiround' });
})();
...
const chatHandler: MockHandler = ({ url, method, body }) => {
	...
	if (path === '/istota/api/chat/rooms' && method === 'POST') {
		const room: MockChatRoom = {
			id: ++mockChatRoomSeq, token: `web-alice-${mockChatRoomSeq}`,
