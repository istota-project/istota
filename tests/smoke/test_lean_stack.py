...
class TestTheStackAnswersATask:
    def test_a_submitted_task_reaches_completed_with_the_scripted_answer(
        ...
        task_id = stack.submit("what is the scripted answer")
    ...
    def test_the_daemon_actually_called_the_endpoint(self, stack):
        ...
        assert stack.endpoint.requests, (
...
class TestTheProbeReadsTheRealSchema:
    def test_the_task_row_carries_the_columns_the_deployment_writes(self, stack):
        ...
        task_id = stack.submit("populate a row")
        task = stack.probe.wait_for_task(
            status="completed", task_id=task_id, timeout=120
        )
