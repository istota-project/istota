...
def _make_app(tmp_path: Path, *, username="alice", admins=("alice",)) -> FastAPI:
...
class TestAdminGate:
    ...
    def test_empty_admins_fails_closed(self, tmp_path):
        app = _make_app(tmp_path, username="alice", admins=())
        c = TestClient(app)
        assert c.get("/istota/api/briefings/shared-blocks").status_code == 403
