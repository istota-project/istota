...
class TestMapEntries:
    def test_basic_entry(self):
        ...
        items = _map_entries(entries, "alice")
        assert len(items) == 1
...
class TestRegenerateFeedPage:
    def test_disabled_site(self, tmp_path):
        ...
        assert regenerate_feed_page(config, "alice", "https://flux.test", "key") is False
    ...
    def test_generates_page(self, mock_fetch, tmp_path):
        ...
        result = regenerate_feed_page(config, "alice", "https://flux.test", "key")
        assert result is True

        output_path = tmp_path / "Users" / "alice" / "istota" / "html" / "feeds" / "index.html"
