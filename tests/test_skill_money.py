...
class TestSetupEnvHook:
    ...
    def _make_ctx(self, tmp_path, *, resources=None, mount=True, user_id="alice"):
    ...
    def test_emits_money_workspace_when_resource_has_no_config_path(self, tmp_path):
        ...
        env = setup_env(ctx)
        expected = cfg.nextcloud_mount_path / "Users" / "alice" / "istota"
    ...
    def test_emits_data_dir_override(self, tmp_path):
        ...
        ctx, _, RC = self._make_ctx(tmp_path)
        ctx.user_config.resources.append(
            RC(type="money", extra={"data_dir": "/srv/money/alice"}),
        )

        env = setup_env(ctx)
        assert env["MONEY_DATA_DIR"] == "/srv/money/alice"
    ...
    def test_emits_config_dir_override(self, tmp_path):
        ...
        ctx, _, RC = self._make_ctx(tmp_path)
        ctx.user_config.resources.append(
            RC(type="money", extra={"config_dir": "/srv/money/alice/config"}),
        )

        env = setup_env(ctx)
        assert env["MONEY_CONFIG_DIR"] == "/srv/money/alice/config"
