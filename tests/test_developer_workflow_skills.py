...
class TestLoadBudget:
    ...
    The ceiling is a ratchet, not a law: moving it is allowed, deliberately and
    with the reason written down, which is what keeps the next person from
    doing it by reflex. It moved once, 675 → 687, for ISSUE-264 — the
    verification budget was telling the model to trim test output without
    saying that a pipe discards the status being trimmed, and a suite that
    exited 1 was twice reported green. Fifteen of those lines are the rule and
    its two remedies; three came back by deleting the third and fourth
    statements of the raw-API write rule, for a net twelve.
    """
