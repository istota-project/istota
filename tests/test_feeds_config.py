...
class TestFetchBriefingEntriesDispatch:
    ...
    def test_miniflux_backend_calls_http_client(self, monkeypatch):
        ...
        items = fetch_briefing_entries("alice", cfg, limit=10)
        assert captured == {
    ...
    def test_native_backend_reads_from_sqlite(self, tmp_path, monkeypatch):
        ...
        cfg.users = {"alice": UserConfig(resources=[ResourceConfig(type="feeds")])}
        ...
        items = fetch_briefing_entries("alice", cfg)
        assert len(items) == 1
    ...
    def test_no_miniflux_resource_returns_empty(self):
        ...
        assert fetch_briefing_entries("alice", cfg) == []
