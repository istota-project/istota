...
- `promoted_room` — `db.create_web_chat_room` (the web handle, the registry row
  ...
  `web_app._chat_promote_to_talk`'s binding write.
...
those producers by running them and diffing the tables, rather than against a
hand-written list of expected rows.
...
def _new_talk_ref() -> str:
    """A Nextcloud Talk token's shape: 8 characters of [a-z0-9]."""
