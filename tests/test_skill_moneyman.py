...
class TestCliMode:
    ...
    def test_run_cli_success(self):
        ...
        env = {"MONEYMAN_CLI_PATH": "/usr/bin/moneyman", "MONEYMAN_USER": "alice"}
        with patch.dict(os.environ, env, clear=True):
            with patch("istota.skills.moneyman.subprocess.run", return_value=mock_result) as mock_run:
                ...
                assert cmd == ["/usr/bin/moneyman", "-u", "alice", "list"]
    ...
    def test_run_cli_with_config(self):
        ...
        with patch.dict(os.environ, env, clear=True):
            with patch("istota.skills.moneyman.subprocess.run", return_value=mock_result) as mock_run:
                _run_cli(["check"])
                cmd = mock_run.call_args[0][0]
                assert cmd == ["/usr/bin/moneyman", "-c", "/etc/moneyman/config.toml", "-u", "alice", "check"]
...
class TestHttpMode:
    ...
    def test_http_client_sets_headers(self):
        ...
        env = {"MONEYMAN_API_URL": "http://localhost:8090", "MONEYMAN_API_KEY": "test-key", "MONEYMAN_USER": "alice"}
        with patch.dict(os.environ, env, clear=True):
            client = _http_client()
            # Key should be HMAC-derived, not the raw master key
            expected_key = _derive_user_key("test-key", "alice")
            assert client.headers.get("X-API-Key") == expected_key
            assert client.headers.get("X-User") == "alice"
            client.close()
...
class TestCommandsCli:
    ...
    def _cli_env(self):
        return {"MONEYMAN_CLI_PATH": "/usr/bin/moneyman", "MONEYMAN_USER": "alice"}
