"""Tests for the per-namespace operator alias-override state (brain._roles).

Covers ``set_alias_overrides`` normalization of the three input shapes,
``get_alias_override_target`` precedence (per-namespace > legacy "*" > None),
...
class TestNormalizeFlat:
    def test_flat_string_stored_under_legacy_namespace(self):
        ...
        rt = get_alias_override_target("smart", LEGACY_NAMESPACE)
    ...
    def test_flat_string_visible_to_any_namespace(self):
        ...
        assert get_alias_override_target("smart", "anthropic").model == "opus-high"
        assert get_alias_override_target("smart", "openai_compat").model == "opus-high"

    def test_flat_blank_dropped(self):
        set_alias_overrides({"smart": "   "})
        assert get_alias_override_target("smart", "anthropic") is None

    def test_role_name_lowercased(self):
        set_alias_overrides({"SMART": "opus"})
        assert get_alias_override_target("smart", LEGACY_NAMESPACE).model == "opus"
...
class TestNormalizePerNamespace:
    def test_namespace_string_values(self):
        ...
        assert get_alias_override_target("smart", "anthropic") == RoleTarget("opus-high", None)
        assert get_alias_override_target("smart", "openai_compat") == RoleTarget(
    ...
    def test_mixed_string_and_table_per_role(self):
        ...
        assert get_alias_override_target("smart", "anthropic") == RoleTarget("opus-high", None)
        assert get_alias_override_target("smart", "openai_compat") == RoleTarget("x/y", "max")

    def test_table_missing_model_dropped(self):
        set_alias_overrides({"smart": {"openai_compat": {"effort": "high"}}})
        assert get_alias_override_target("smart", "openai_compat") is None

    def test_table_blank_model_dropped(self):
        set_alias_overrides({"smart": {"openai_compat": {"model": "  "}}})
        assert get_alias_override_target("smart", "openai_compat") is None

    def test_effort_blank_normalized_to_none(self):
        set_alias_overrides({"smart": {"anthropic": {"model": "opus", "effort": "  "}}})
        assert get_alias_override_target("smart", "anthropic") == RoleTarget("opus", None)
...
class TestPrecedence:
    def test_per_namespace_wins_over_legacy(self):
        ...
        set_alias_overrides({"smart": {"anthropic": "opus", LEGACY_NAMESPACE: "sonnet"}})
        assert get_alias_override_target("smart", "anthropic").model == "opus"
        assert get_alias_override_target("smart", "openai_compat").model == "sonnet"

    def test_namespace_miss_no_legacy_returns_none(self):
        set_alias_overrides({"smart": {"anthropic": "opus"}})
        assert get_alias_override_target("smart", "openai_compat") is None

    def test_unknown_role_returns_none(self):
        set_alias_overrides({"smart": "opus"})
        assert get_alias_override_target("general", "anthropic") is None


class TestMalformedAndReset:
    def test_non_string_role_key_dropped(self):
        set_alias_overrides({123: "opus", "smart": "sonnet"})
        assert get_alias_override_target("smart", LEGACY_NAMESPACE).model == "sonnet"

    def test_non_string_non_table_value_dropped(self):
        set_alias_overrides({"smart": 42, "fast": "haiku"})
        assert get_alias_override_target("smart", "anthropic") is None
        assert get_alias_override_target("fast", LEGACY_NAMESPACE).model == "haiku"

    def test_non_string_namespace_key_dropped(self):
        set_alias_overrides({"smart": {123: "opus", "anthropic": "sonnet"}})
        assert get_alias_override_target("smart", "anthropic").model == "sonnet"
    ...
    def test_none_clears_table(self):
        set_alias_overrides({"smart": "opus"})
        set_alias_overrides(None)
        assert get_alias_override_target("smart", "anthropic") is None

    def test_atomic_rebind_replaces_not_merges(self):
        set_alias_overrides({"smart": "opus", "fast": "haiku"})
        set_alias_overrides({"smart": "sonnet"})
        assert get_alias_override_target("smart", LEGACY_NAMESPACE).model == "sonnet"
        # The prior "fast" entry is gone — rebind is a replace, not a merge.
        assert get_alias_override_target("fast", "anthropic") is None


class TestGetRoleOverrides:
    def test_returns_role_names_as_keys(self):
        set_alias_overrides({"smart": {"anthropic": "opus"}, "deep": "sonnet"})
        table = get_alias_overrides()
