...
class TestPublish:
    def test_authorized_write(self, tmp_path):
        ...
        with db.get_db(cfg.db_path) as conn:
            job = _make_job(conn, "alice", publish="film-digest", trusted=False)
    ...
    def test_trusted_flag_written(self, tmp_path):
        ...
        class _T:
            user_id = "alice"
        with db.get_db(cfg.db_path) as conn:
            job = _make_job(conn, "alice", publish="markets", trusted=True)
    ...
    def test_ns_key_target(self, tmp_path):
        ...
        class _T:
            user_id = "alice"
        with db.get_db(cfg.db_path) as conn:
            job = _make_job(conn, "alice", publish="custom/key1")
    ...
    def test_unauthorized_fails_loud(self, tmp_path, monkeypatch):
        ...
        class _T:
            user_id = "alice"
        with db.get_db(cfg.db_path) as conn:
            job = _make_job(conn, "alice", publish="film-digest")
