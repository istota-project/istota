...
- `src/istota/config.py` — `imap_timeout_seconds`, `quiet_email_senders`, `is_quiet_email_sender`.
- `src/istota/user_profiles.py`, `src/istota/db.py`, `schema.sql` — profile column + migration.
- `src/istota/cli.py`, `src/istota/web_app.py`, `web/src/{lib/api.ts,routes/settings/+page.svelte}` — `--quiet-sender` + settings card.
- `deploy/ansible/*`, `config/config.example.toml` — Ansible wiring + example config.
- `tests/test_skills_email_client.py`, `tests/test_skills_email_send.py`, `tests/test_email_quiet_senders.py` (new) + `tests/test_skills_email.py`, `tests/test_cli_user_ensure.py`.
...
- **Fallback + circuit breaker.** A launch-level failure returns `stop_reason="fallback"` (missing tmux → `not_found`); the executor reruns that same attempt once through `claude_code` (no new DB row). A process-global `_CircuitBreaker` opens after N consecutive launch failures, short-circuits to fallback for a cooldown, and arms one operator alert (the executor pops it via `consume_circuit_open_alert()` and fires `send_notification(purpose="alert")`, since the brain has no `Config`). Any tmux success resets it.
- **Live transcript tailing (streaming recovery).** Because the full switch puts interactive chat on this brain, whole-turn-only delivery would regress web/repl streaming. A background `_TranscriptTailer` tails the transcript JSONL during the turn and forwards tool/text/thinking blocks live (deduped by tool id + block index). The Stop-time parse stays authoritative for the persisted result/trace (`_build_result(forward_progress=tailer is None)` avoids double-emit). Tier 1 (block-level) shipped; Tier 2 (token-level) deferred to a live partial-flush probe. The brain can't distinguish push (Talk) from stream (web/repl) surfaces — `req` carries no surface — so the tailer runs whenever `streaming`; push consumers coalesce identically.
- **Config + observability.** New `TmuxBrainConfig` (`[brain.tmux]`, on `BrainConfig.tmux`), all fields defaulted to the prototype's hardcoded constants (empty block = behavioral parity): fallback threshold/cooldown, ready/command timeouts, CLI-version pin, and the readiness/dialog/error marker lists (so a CLI reword is a config hotfix). `make_brain` passes it; `validate_config.py` allows + shape-checks the block. One structured `tmux_brain session=… outcome=… ready_ms=… wait_ms=… dialogs=… tools=… retries=…` INFO line per attempt. The executor sets `session_label=f"istota-{task.id}-{task.attempt_count}"` (retry sessions get a `-rN` suffix). A CLI-version-pin mismatch warns once at brain construction.
- **Docs + Ansible.** `brain.md` gained a full TmuxClaudeBrain section; `config.md`, AGENTS.md (now "three brains"), and `config.example.toml` updated. Ansible surfaces `istota_brain_tmux_*` defaults, templates `[brain.tmux]` (emitted on a full switch or a tmux route), and adds a non-fatal CLI-version-pin check task.
...
- `src/istota/brain/_types.py` - document `stop_reason="fallback"`
- `src/istota/config.py` - `TmuxBrainConfig` dataclass + `[brain.tmux]` parsing
- `src/istota/executor.py` - `session_label` wiring + in-attempt headless fallback rerun + circuit-open alert
- `deploy/ansible/{defaults/main.yml,templates/config.toml.j2,files/validate_config.py,tasks/main.yml}` - tmux knobs, template block, shape validation, CLI-version check task
- `.claude/rules/brain.md`, `.claude/rules/config.md`, `AGENTS.md`, `config/config.example.toml` - docs
- `tests/test_tmux_production.py` (new), `tests/test_executor_streaming.py`, `tests/test_tmux_execute.py`, `tests/test_brain_tmux.py`
...
**Spec re-baseline first.** The spec was drafted 2026-05-27, before the transport-abstraction and task-event-streaming refactors landed (both 2026-06-06). Three of its assumptions were stale: `talk_poller.py` is gone (`poll_talk_conversations` lives in `transport/talk/inbound.py`); `_make_talk_progress_callback` is gone (progress-to-Talk is now the event-streaming consumers); and `TalkClient` is built through the `TalkTransport` seam plus `notifications`/consumers/`commands`/inbound, not one place. Re-baselined the call-site map, Stages 3/5, and the test-file names, and recorded the three confirmed decisions (process-global singleton client; full daemon-process scope incl. consumers + notifications; rewrite-then-implement) before touching code.

**The components.** `async_runtime.py` adds `AsyncRuntime` (the loop thread + a `submit(coro, timeout=None)` sync→async bridge over `run_coroutine_threadsafe`, with a reentry guard that raises instead of deadlocking if called from the loop's own thread), the `run_coro` workhorse every sync Talk call site now uses, and `get_talk_client(config)` — a process-global persistent `TalkClient` singleton that the transport seam, the event consumers, `notifications._send_talk`, the inbound poller, `commands.dispatch`, `_resolve_channel_name`, and `_finalize_log_channel` all pull from so they share one connection pool. `TalkClient` grew a lazily-created `self._client`, `_ensure_open`, `aclose`, and `is_closed`; its 11 methods dropped their per-call `async with httpx.AsyncClient(...)` and now issue requests on `self._client` (methods that needed a longer deadline pass `timeout=` per request, which httpx honours).

**The load-bearing invariant.** Once the methods bind to the persistent loop's `self._client`, **every** `TalkClient` method call must run on that loop, or it'd await a loop-bound client from a foreign loop. So the migration order mattered: Stages 3–5 moved every call site to `run_coro` (consumers, the `post_result_to_talk`/`edit_talk_message` shims, `TalkTransport.deliver`/`.edit`, `notifications`, the poller, inbound, commands) while methods still used transient clients; only then did Stage 6 flip the methods to `self._client`. There are no transient `TalkClient(config)` constructions left in any daemon Talk path. Email delivery stays on `asyncio.run` (sync SMTP, not httpx). `run_cleanup_checks` became synchronous — its two rare Talk notices route through `send_notification` (→ `run_coro`) so its blocking DB/IMAP/fs cleanup never lands on the persistent loop.

**Reentry trap, caught early.** The first cut of `get_talk_client` eagerly opened the client via `run_coro` — which deadlock-guards when called from the loop thread, exactly what happens when `TalkTransport.deliver` (running on the loop) calls `get_talk_client` synchronously. Fixed by making the accessor reentry-safe: it never calls `run_coro`; the httpx pool opens lazily on the first awaited method call, which is always on the persistent loop because every call site goes through `run_coro`.

**The test hang.** Mid-way the suite started hanging. `TestTalkPollThread` controlled `_talk_poll_loop` by patching `scheduler.asyncio.run` with a side-effect that set `_shutdown_requested = True`; once the loop called `run_coro` instead, the patch never fired and the `while not _shutdown_requested` loop spun forever, pinning a CPU. (Compounded by my own operator error — I'd stacked five overlapping `pytest -n auto` runs while chasing it, saturating the machine.) Fix: repoint the three poll-loop tests to `run_coro`. Bisected it with a verbose run written straight to a file — piping a hanging pytest through `tail` shows nothing because `tail` buffers until EOF.

**Lifecycle + shutdown.** `run_daemon` starts the runtime explicitly before the talk-poller thread (so a loop-creation failure surfaces before going live) and stops it after the worker pool: `stop()` runs cleanup hooks (closing the shared client) then stops the loop, cancelling any in-flight long-poll, best-effort under a 10s timeout so a hung coro can't block shutdown. Single-pass `run_scheduler` shares `process_one_task` (now on `run_coro`), so it lazily uses the same runtime; its one-shot process exits with the daemon thread.

**Tests.** Full suite green (5582 passed). New `tests/test_async_runtime.py` (lifecycle, submit, timeout/cancel, reentry, concurrency, cleanup hooks) and `tests/test_talk_client_persistent.py` (singleton, lazy-open, connection reuse, post-aclose raise, per-request timeout override, a Stage-3 reuse proof, a Stage-5 poll-cycle-via-`run_coro` test). Existing `test_talk.py` mocks kept working unchanged — the `httpx.AsyncClient` patch still intercepts construction inside `_ensure_open`. Only Stage 10 (deploy + observe socket reuse on the host) remains.
...
**ISSUE-097: command-type scheduled jobs missing `dispatch_setup_env_hooks`.** Commit `e19f46d` (2026-05-15) added the hook dispatch to `scheduler._execute_skill_task` to fix the equivalent issue for `HEALTH_DB_PATH` on the auto-seeded module-skill path. The same fix didn't reach `_execute_command_task`, which is what operator-defined CRON.md `command:` rows go through. `gym-check-sync` had been working anyway because `LOCATION_DB_PATH` happened to be in the daemon's systemd environment from before the hook era; a daemon restart between the 2026-05-18 06:05 run and the 2026-05-19 run lost it. One-line fix: `env.update(dispatch_setup_env_hooks(list(skill_index), skill_index, ctx))` after the existing `build_skill_env` call. Used `env.update(...)` (not `if k not in env`) because hook values are per-user and authoritative — a stray ambient `LOCATION_DB_PATH` left over from systemd would point at the wrong user's DB.

**ISSUE-098: cron Garmin sync couldn't decrypt tokens from a subprocess.** Same `e19f46d` moved Garmin OAuth tokens from plaintext `health_settings` into the encrypted secrets table (Fernet via `ISTOTA_SECRET_KEY`). The subprocess env strips that key by design — `executor.build_clean_env` enforces `_PROXY_LOOKUP_BLOCKED`, defense-in-depth so a compromised skill subprocess can't decrypt every secret for the user. The engine reaches `secrets_store.get_secret` → `secret_key_available()` → returns `None` → "no Garmin tokens." The web `/garmin/sync` endpoint already calls the engine in-process and works fine. Considered injecting just the Garmin blob via env (the issue's proposed fix), but the engine re-encrypts rotated SDK tokens mid-run plus writes an error flag and `last_sync` — a single pre-resolved injection wouldn't cover the round trip. Took the alternative: short-circuit `skill="health"` + `skill_args[0]=="garmin-sync"` in `_execute_skill_task` into a new `_run_garmin_sync_inprocess` helper that calls the engine on the daemon thread, mirroring the web path. Same `(success, result_text)` shape the subprocess returned, so the scheduler delivery / failure tracking flows are unchanged.

**Mulder + Scully review surfaced ISSUE-099 (filed as a same-shape extension, not a separate issue).** Heartbeat's `_check_shell_command` had the identical setup_env-hook gap — only `build_stripped_env()`, no `build_skill_env`/`dispatch_setup_env_hooks` — and worse, the no-condition branch evaluates `returncode == 0` as healthy. A `shell-command` heartbeat invoking `istota-skill location current` would see `LOCATION_DB_PATH` unset, the skill CLI would print `{"status":"error","error":"…"}` to stdout while exiting 0, and the heartbeat would silently report healthy. Bundled fix: new `_build_heartbeat_skill_env(config, user_id)` helper constructs a minimal `EnvContext` (fake `db.Task`, `discovered_calendars=[]` to skip the PROPFIND-per-tick cost Mulder flagged on the scheduler path) and dispatches both `build_skill_env` and `dispatch_setup_env_hooks`. `_check_shell_command` calls it when `user_id` is set, swallowing exceptions with a WARN so a hook crash degrades to "subprocess sees no skill env" rather than "heartbeat dies." The no-condition branch now parses the JSON error envelope as failure, matching `_execute_command_task`'s defense-in-depth.

**Architectural conversation followed: should `istota-skill health garmin-sync` also work from a developer shell?** Mulder's HIGH finding was that the CLI subcommand is still callable from a hand-written CRON `command:` row, an LLM Bash invocation, or a dev shell — all three still get the misleading `token_expired` because the master key isn't there. Four options weighed (fail-loud, enqueue-and-poll, CLI-calls-web-endpoint, extend the skill proxy with a `run_in_daemon` verb). Wrote a project note (`Notes/Projects/Istota/Skill proxy execution model and the master-key boundary.md`) covering why the master key is structurally different from a per-service credential (blast-radius — Fernet-at-rest exists so a compromised health subprocess can't decrypt Monarch cookies), why Garmin's lifecycle is awkward where most other skills aren't (decrypt → call → re-encrypt → store round trip rather than read-once), and the break-even math for building the mechanism vs one-shotting the case. Picked one-shot (enqueue-and-poll) — break-even on the mechanism is ~3 cases and we have one. The deeper insight from the conversation is that the "skill proxy" is misnamed: it's already a daemon-side execution channel with credential injection as the current strategy. Note records the rename + comment-pointer follow-up for when a third case forces the refactor.
...
- `cmd_garmin_sync` split into direct/delegated router + helpers. Delegated mode enqueues a `skill="health"` task with `max_attempts=1` and polls; clear error envelopes for sandbox/timeout/missing-env cases.
- ISSUE-097 / 098 closed in `_ISSUES.md` with resolution notes.
...
- `tests/test_scheduler.py` — `TestGarminSyncInProcess` (5) + 2 new `TestExecuteCommandTask` cases.
- `tests/test_heartbeat.py` — 9 new `TestCheckShellCommand` cases.
- `tests/test_health_skill.py` — `TestGarminSyncRouting` (2) + `TestGarminSyncDelegated` (7).
- `CHANGELOG.md` — `[Unreleased]` entries for routing change + three Fixed items.
...
The shape: pre-refactor, credentials lived in four parallel places. The executor hand-injected ~30 env vars from `config.*` for every task. `_PROXY_CREDENTIAL_VARS` listed the sensitive subset for the skill-proxy split. `_CREDENTIAL_SKILL_MAP` mapped each var to the skills that consume it (used for proxy authorization). And `_authorized_skills_from_credentials` walked that map to compute the authorized set. Every new credential needed an edit in three of the four. New skills needed an edit in two. Doc-only skills (developer, ntfy) needed special-cased entries that weren't actually consumed via env. The `ISTOTA_SECRET_KEY` master Fernet key was leaking into subprocess envs because `_STRIPPED_ENV_PRESERVE` had whitelisted it for module-skill subprocesses to fetch their own secrets. End-state goal: the skill manifest is the only place credentials are declared. Everything else derives.
...
**Phase 4 — cron module-job dispatch cleanup.** Operator CRON.md `command:` rows of the shape `istota-skill <name> [args]` (no shell metacharacters, no env-var prefix, no quoting tricks) auto-promote to skill-tasks at sync time via `_parse_skill_command` / `_resolve_job_dispatch` / `fj_is_disallowed_command`. They dispatch through `_execute_skill_task` (no admin gate) instead of `_execute_command_task` (admin-only). `migrate_db_jobs_to_file` round-trips skill-task rows back to a single `command:` line so the file stays operator-editable and idempotent.

**Gaps Mulder caught after Phase 4 landed.** A two-agent verification pass (mulder + scully) ran the spec against the diff. Scully signed off on Phases 1–4 with file:line citations and 490/490 in the targeted suite. Mulder found three latent issues:

1. `_execute_skill_task` was building `EnvContext` without populating `discovered_calendars`. The calendar / location manifests gate `CALDAV_*` on `gate_has_discovered_calendars`. Today's auto-seeded skill-tasks (feeds, money) don't touch CalDAV, so nothing broke — but the moment an operator promotes a CalDAV-touching skill-job in CRON.md, credentials would silently disappear.
2. `_execute_skill_task` was calling `build_skill_env([skill_name], skill_index, ctx)` — env resolution scoped to the requested skill's manifest only. `NC_URL` is declared on both `files` (always-include) and `nextcloud`. If a future operator promotes any skill-task that needs `NC_URL`, it'd be missing. Same shape latent.
3. `_execute_command_task` still hardcoded `NC_URL`/`NC_USER`/`NC_PASS`/`CALDAV_URL`/`CALDAV_USERNAME`/`CALDAV_PASSWORD` — the very pattern Phase 2 deleted from the executor.

Fix: extracted `discover_calendars_for_task(task, config)` from `executor.execute_task` (best-effort, returns `[]` on any failure). Both subprocess paths now build env via `build_skill_env(list(skill_index), skill_index, ctx)` over an `EnvContext` populated by that helper. Three behavior consequences worth noting: (a) skill-tasks now receive every co-declared env var that resolves under their context, not just the requested skill's; (b) command-tasks now honor `gate_has_discovered_calendars` — `CALDAV_*` is omitted when discovery returns empty, matching the LLM path; (c) the test `test_caldav_env_vars_passed` had to mock `discover_calendars_for_task` because its synthetic CalDAV URL wouldn't actually discover anything.

**Test status.** 4339 passed, 7 skipped. New `tests/test_credential_derivation.py` (373 cases) pins the four derivation helpers' invariants — sensitive-only collection, fallback_var doesn't auto-authorize, perf budget, conflict semantics. `test_skill_env.py::TestExecutorHardcodedCredentialBlockGone` greps `executor.py` so a future commit can't quietly re-introduce hardcoded credential injection. New scheduler tests for the gap-fix work — co-declared resolution, calendar-gate positive/negative for both subprocess paths.
...
- `src/istota/db.py`, `schema.sql` — `tasks.skill` / `tasks.skill_args` columns + migrations.
- `src/istota/skills/_env.py`, `_types.py`, `_loader.py` — `EnvSpec` extensions; `from: "setup_env"` and `from: "secret"` sources; `EnvContext.discovered_calendars`; `gate_*` pre-filters; `dispatch_setup_env_hooks` iterates full index.
- `src/istota/skills/{calendar,email,nextcloud,files,bookmarks,developer,google_workspace,location,money,feeds,ntfy}/skill.md` — manifests now declare every credential they consume, with `sensitive`/`fallback_var`/`gate_*` as appropriate.
- `src/istota/heartbeat.py`, `src/istota/cli.py` — `build_stripped_env` no longer special-cases anything.
- `src/istota/feeds/_loader.py`, `src/istota/feeds/jobs.py`, `src/istota/money/_loader.py`, `src/istota/money/jobs.py` — env-first credential reads via `_read_credential`.
...
- **Object length cap.** New `MAX_FACT_OBJECT_CHARS = 100` constant; `_validate_fact` rejects objects longer than that. Prompt explicitly says "Object values MUST be under 10 words (max 100 characters) — put context in MEMORIES, not in fact objects." Long objects break fuzzy dedup (rewordings drift past Jaccard 0.6) and crowd out other prompt content once the existing-facts section is loaded.
- **Predicate vocabulary expanded.** Added `traveled_to`, `has_appointment`, `has_family_member`, `interested_in`, `completed`. Each gets a hint string; `traveled_to` and `has_appointment` mention `valid_from`/`valid_until` use. Existing predicates kept as-is.
- **`decided` expiry guidance.** Prompt now distinguishes durable decisions (no `valid_until`) from one-time actions / cancellations / purchases (set `valid_until` a few weeks out so they age out of the current-fact view). Aimed at the specific cleanup pattern the user has been doing manually — stale `cancel lax-waw lisbon trip` and `investigate claude 4.7 migration` `decided` entries.
...
- `AGENTS.md` — file-tree entry for `sleep_cycle.py` removed from top-level, added to `memory/`; cross-reference updated.
- `docs/architecture/memory.md`, `docs/architecture/overview.md`, `docs/features/memory.md`, `.claude/rules/scheduler.md` — path references updated.
- `Notes/Projects/Istota/KG sleep cycle analysis and improvement plan.md` (outside the repo) — status update appended per its frontmatter `agents:` directive.

## 2026-04-28: Memory subsystem round 2 — brain routing, KG audit, per-chunk topics, recall dedup
...
- **Knowledge graph audit log.** New `knowledge_facts_audit` table (`user_id, fact_id, op, before_json, after_json, source_task_id, source_type, ts`). Every KG mutation writes a row: `add_fact` emits one of `insert` / `supersede` / `fuzzy_dedup_skip`; `invalidate_fact` and `delete_fact` emit `invalidate` / `delete`. Pruned at 4× the user's `memory_retention_days` in the user sleep cycle. New CLI `istota-skill memory_search fact-history [--entity X] [--since DATE] [--limit N]`.
- **`max_knowledge_facts` default 0 → 50; smarter truncation.** Identity facts (subject == user_id) are anchors and never truncated, even when their count exceeds the cap. Matched facts (subject/object in prompt) sort by `updated_at` desc with `created_at` tiebreak; debug log when truncation drops anything. If identity alone overruns the cap, identity wins and matched facts are dropped — the over-budget state is surfaced in logs rather than hidden by silent identity truncation.
- **KG fuzzy-dedup tightened.** Threshold 0.7 → 0.6, plus a token-subset fast-path scoped to identical predicates: one object's whitespace-tokens being a subset of the other's collapses them. Catches `python` ⇄ `python 3` and `acme` ⇄ `acme corp`; rejects cross-predicate collisions (`is_allergic_to` vs `allergic_to`) and numeric-prefix collisions (`tech_1` vs `tech_10` — different single tokens, not a subset). Audit row records `match_type: "token_subset"` vs `"jaccard"` so threshold tuning has signal.
- **Per-chunk topics in dated memory indexing.** The model already classifies per-task topics in the extraction's TOPICS section; previously we collapsed to a single dominant topic per file. New `_topics_per_chunk(chunks, extracted_topics)` walks each chunk for `ref:N` markers and assigns the topic of the first ref it contains; chunks without a ref get None (NULL-topic chunks are always returned in topic-filtered searches by design). `_insert_chunks` and `index_file` accept `topic_per_chunk: list[str | None] | None`.
- **User-tz dated filenames.** `process_user_sleep_cycle` resolves `user_config.timezone` via `ZoneInfo` and computes `date_str` in user-local time. Channel cycle stays UTC because channels span timezones. Bullet timestamps inside the file use the same date string.
- **USER.md size observability (phase A).** Curation audit JSONL now records `user_md_size_bytes` per run. `_maybe_warn_usermd_size` posts a one-line warning to the user's `log_channel` when USER.md crosses 8 KB. `memory_search stats` surfaces `user_md_size_bytes` (None when the file doesn't exist). Active size pressure (phase B — hard cap, summarize op, decay) is deferred until phase A produces real growth-curve data.
...
- `src/istota/memory/search.py` — `index_file` / `_insert_chunks` accept `topic_per_chunk`; `search` accepts `exclude_conversation_task_ids`; `reindex_all` indexes durable CHANNEL.md.
- `src/istota/memory/curation/audit.py` — `write_audit_log` accepts `user_md_size_bytes`.
- `src/istota/executor.py` — `_build_talk_api_context` and `_build_db_context` return `(text, task_ids)`; `_recall_memories` accepts `exclude_task_ids` and queries `channel_memory_durable` in the channel namespace; `execute_task` threads the set through.
- `src/istota/skills/memory_search/__init__.py` — `cmd_fact_history` + parser, `_user_md_size_bytes` in stats.
- `src/istota/config.py` — `max_knowledge_facts` default 0 → 50; `SleepCycleConfig.extraction_model` / `curation_model`; `ChannelSleepCycleConfig.extraction_model`.
- `schema.sql` — `knowledge_facts_audit` table + indexes.
- `tests/test_sleep_cycle.py`, `tests/test_channel_sleep_cycle.py` — bulk-rewritten to patch `_run_sleep_cycle_brain`; new tests for user-tz filenames, brain routing, durable CHANNEL.md indexing, `_topics_per_chunk`, `TestUserMemoryObservability`.
- `tests/test_knowledge_graph.py` — `TestKnowledgeFactsAudit` class (insert/supersede/fuzzy_dedup_skip/invalidate/delete/history/retention), tightened fuzzy-dedup tests, `select_relevant_facts` tests rewritten for the no-truncate-identity rule.
- `tests/test_memory_search.py` — `exclude_conversation_task_ids` filter tests.
- `tests/test_executor.py` — updated mocked context builder return shape.
- `docs/features/memory.md` — top-region constraint section with worked layout example.
- `AGENTS.md`, `.claude/rules/brain.md` — KG dedup / audit / cap, sleep-cycle brain routing, empty-allowed_tools note.

3619 tests pass.

## 2026-04-28: Memory subsystem reorganization, op-based USER.md curation, unified retention

A 105 MB prod DB snapshot showed 80% of the database was the memory_search subsystem (84 MB across `memory_chunks` + the vec/FTS shadow tables), with two structural problems: silent scheduled jobs were auto-indexing for nothing (99.4% of scheduled tasks; no recall value) and 92% of conversation chunks were orphans whose source tasks had been retention-cleaned but whose chunks lingered forever. On top of that, the optional `curate_user_memory` pass rewrote USER.md from scratch every night via Sonnet — wrong architecture for a durable file, prone to silent drift and quiet deletions.
...
- **Op-based USER.md curation.** New package at `src/istota/memory/curation/` with five modules:
  ...
  - `parser.py` — `parse_sectioned_doc` splits on `^## `, treats `### ` and below as opaque content inside `Section.lines`. `serialize_sectioned_doc` is the inverse with a guaranteed single trailing newline.
  - `ops.py` — three op kinds: `append` (heading must exist; case-insensitive dedup against existing top-region bullets emits `noop_dup`; line is normalized to `- text`), `add_heading` (heading must NOT exist; new section appended to end; bullets normalized), `remove` (case-insensitive substring match against bullet text; zero matches → `noop_no_match`; multiple matches → rejected with `multiple_matches`). Applier deep-copies the input doc once at entry; bad ops accumulate in `rejected` while good ones still apply. Never raises.
  - `prompt.py` — builds the curation prompt (current doc structure + last 3 days of dated memories + KG facts + per-op rules) and `strip_json_fences` to tolerate ` ```json … ``` ` wrapping.
  - `audit.py` — append-only sidecar `USER.md.audit.jsonl` next to USER.md. One JSONL line per night that produced ops; nights with neither applied nor rejected don't write.
- **Curation orchestrator rewritten.** `sleep_cycle.curate_user_memory` is now a ~50-line script over the curation package. Reads current USER.md, parses, builds prompt with KG facts, calls Sonnet, strips fences, parses JSON, applies ops, writes only when at least one op had outcome `"applied"`, audit-logs the run, re-indexes via `memory.search.index_file`, and posts a one-line summary to the user's `log_channel` (gated by new `SleepCycleConfig.curation_log_summary` default true). The previous `build_curation_prompt` and `NO_CHANGES_NEEDED` constant are gone.
- **Unified memory retention.** New `cleanup_old_chunks(conn, user_id, retention_days, source_types=None)` in `memory/search.py`. Default ephemeral set is `('conversation', 'memory_file', 'channel_memory')` — `user_memory` deliberately excluded as durable. Cascades to `memory_chunks_vec` per-row (no trigger exists), relies on the existing FTS5 trigger for `memory_chunks_fts`. Wired into `process_user_sleep_cycle` after `cleanup_old_memory_files` and into `process_channel_sleep_cycle` scoped to `('channel_memory',)`. Reuses the existing `[sleep_cycle] memory_retention_days` knob — one knob, uniform effect. Step 0 of the plan (verify durable CHANNEL.md isn't sharing source_type with dated channel files) confirmed durable CHANNEL.md isn't indexed at all today, so no source_type refactor was needed.
- **Mulder/Scully review fixes (separate commit).** Five findings from an investigative + verification review:
  1. **Critical: timestamp format mismatch in `cleanup_old_chunks`.** The cutoff used Python's `isoformat()` (`'2026-04-28T07:54:44.283261'`), but SQLite's `datetime('now')` column default produces `'2026-04-28 07:54:42'` — SPACE separator, second precision. Lex-comparison flipped: ASCII space (0x20) < `'T'` (0x54), so up to 24 hours of rows on the cutoff day got incorrectly classified as old. The existing tests passed because the test helper backdated rows via Python's `isoformat()` — same buggy format as the cutoff. Fixed by switching to `strftime('%Y-%m-%d %H:%M:%S')` and added two regression tests that exercise the production write path: insert via the column default + age via SQLite's own `datetime('now', '-N days')`.
  2. **Vec-cascade silent partial commit.** The vec-row `DELETE` loop sat inside one outer try/except — on first failure, every subsequent delete was skipped, then `conn.commit()` committed the partial vec state alongside the full main-table delete, leaving orphan vec rows. Moved try/except inside the loop.
  3. **Drift-induced spurious USER.md rewrites.** The "skip write when nothing changed" guard compared serialized output against `current` text. USER.md hand-edits with trailing whitespace on headings, missing trailing newline, or CRLF line endings get normalized through the parse/serialize round-trip — so even all-noop op runs (every dedup) would write back a cleaned file every night. Replaced text comparison with an outcome check: skip write iff every applied op had outcome `noop_dup`/`noop_no_match`. Regression test added.
  4. **`append` fuses onto trailing paragraph.** New bullet inserted after a section-ending paragraph had no blank gap, visually fusing in rendered markdown. Applier now inserts a blank line when the predecessor in the top region is a paragraph; bullet→bullet adjacency unchanged.
  5. **Hash-prefix rejection too aggressive.** `_validate_appendable_line` rejected any line whose body started with `#` — caught legitimate content like `"#hashtag"`, `"#footnote-1"`, `"See issue #42"`. Narrowed to `^#{1,6}(\s|$)` (actual heading shapes only).
...
- `src/istota/memory/curation/{__init__.py,types.py,parser.py,ops.py,prompt.py,audit.py}` — new modules for op-based curation.
- `src/istota/sleep_cycle.py` — `curate_user_memory` rewritten end-to-end; `_load_kg_facts_text` and `_post_curation_summary` helpers added; `cleanup_old_chunks` wired into both user and channel sleep cycles. Dead code (`build_curation_prompt`, `NO_CHANGES_NEEDED`) removed.
- `src/istota/scheduler.py` — `process_one_task` now skips memory-search auto-indexing for `task.heartbeat_silent`.
- `src/istota/config.py` — `SleepCycleConfig.curation_log_summary` field.
- `src/istota/commands.py`, `src/istota/executor.py`, `src/istota/skills/memory_search/__init__.py` — import paths updated to new memory subsystem layout.
- `src/istota/geo.py` — `dedupe_near_duplicate_pings`, `validate_cluster_places`. `_finalize_cluster` now picks place_id from the first member ping with one set (was always the first ping).
- `src/istota/skills/location/__init__.py`, `src/istota/web_app.py` — wire the two new geo helpers into the day-summary path; SELECT places.id alongside name/lat/lon/radius for the validator.
- `tests/test_curation_{types,parser,ops,prompt,audit}.py` — 5 new test files, ~85 cases.
- `tests/test_sleep_cycle.py` — `TestCurateUserMemory` rewritten for op-based curation; `TestSleepCycleChunkCleanup` added; drift regression test added.
- `tests/test_channel_sleep_cycle.py` — `TestChannelSleepCycleChunkCleanup` added.
- `tests/test_memory_search.py` — `TestCleanupOldChunks` added (11 cases including timestamp-format regression).
- `tests/test_memory_search.py`, `tests/test_knowledge_graph.py`, `tests/test_skills_memory_search.py`, `tests/test_commands.py`, `tests/test_executor.py`, `tests/test_scheduler.py`, `tests/test_channel_sleep_cycle.py` — import paths updated.
- `tests/test_location.py` — coverage for `dedupe_near_duplicate_pings` and `validate_cluster_places`.
- `config/config.example.toml`, `deploy/ansible/defaults/main.yml`, `deploy/ansible/templates/config.toml.j2` — `curation_log_summary` defaults; expanded `memory_retention_days` documentation.
- `AGENTS.md` — sleep-cycle and memory-search sections updated; project tree updated for the `memory/` subdirectory.
...
- `tests/test_skills_loader.py` — new `TestUntrustedInputCompanion` class.

## 2026-04-26: Fall back to user's resolved channel for email-task Talk delivery

A user reported a Zorg behavior that resolved to a structural bug: a thread-match email task delivered an outbound SMTP reply to an external contact, but the corresponding Talk notification (which `output_target="both"` was supposed to also produce) silently never appeared. The user only saw a contextless follow-up email from Zorg later.
...
**Log channel observability.** Final-state log channel header now ends with the resolved spec inline — e.g. `**[#42]** ✅ Done (3 actions) - cli (claude-opus-4-7 high)` — using the resolved values (task override or config default). When only one of the two is set, the parens contain just that value. Saves a line versus a separate `Model:`/`Effort:` block while keeping per-job overrides visible at a glance. When CRON-overridden Sonnet/low jobs start landing, the log channel makes that visible without having to mentally cross-reference CRON.md. `_format_log_channel_body` and `_finalize_log_channel` gained `model` / `effort` kwargs; `process_one_task` does the resolve.
...
- Ansible: `secrets.env.j2` template deployed to `/etc/istota/secrets.env` (root:zorg, 0640), `EnvironmentFile=` in scheduler service, passwords conditionally omitted from config.toml when `zorg_use_environment_file: true`
...
- `ansible: tasks/main.yml` — Deploy `/etc/istota/admins` file
