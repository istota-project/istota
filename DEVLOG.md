...
- **`decided` expiry guidance.** Prompt now distinguishes durable decisions (no `valid_until`) from one-time actions / cancellations / purchases (set `valid_until` a few weeks out so they age out of the current-fact view). Aimed at the specific cleanup pattern the user has been doing manually — stale `cancel lax-waw warsaw trip` and `investigate claude 4.7 migration` `decided` entries.
...
Three commits address all of this. Plan source: `/Users/stefan/Dust/Notes/Projects/Istota/Memory system fixes and enhancements plan.md`.
