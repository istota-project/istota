...
-- The per-user credential vault's two selecting fields, when they come from a
-- surface rather than from `config.toml`.
--
-- **A table of its own, and that is a security control rather than a
-- placement.** Every other per-user scalar is overlaid from `user_profiles`,
-- which is the general settings store. These two are not preferences:
-- `vault_path` selects which KDBX file the daemon decrypts with a key it holds,
-- and `vault_services` selects which of that user's stored credentials the file
-- may overwrite and delete. `user_profiles` is reachable by anything that grows
-- a profile-writing verb; this table is written by the web settings endpoint
-- and `istota user ensure` and by nothing else, and
-- `tests/test_user_vault_config.py` sweeps the tree rather than naming its
-- subjects so a third writer fails a test instead of going unnoticed.
--
-- A row present is the authority; no row falls back to `[users.<id>]`. Clearing
-- deletes the row rather than blanking it, because an empty `vault_path` and no
-- row are the same state — "the feature is off for this user" — and two
-- spellings of one state is how a precedence rule starts lying: a blank row
-- winning over a TOML line would make an operator's config silently inert.
--
-- `vault_path` here is always *relative*, resolved under the user's own
-- workspace. The absolute form is the escape hatch that puts the file outside
-- every sandbox bind and is checked against `_sandbox_writable_roots` rather
-- than against one user's tree, so it stays operator-only in TOML; a
-- user-writable absolute path would be an arbitrary read as the daemon user.
-- The endpoint refuses one, and this comment is where that rule is written
-- down rather than a CHECK, since the containment question the refusal really
-- asks is `storage.resolve_user_vault_path`'s and not expressible here.
--
-- The passphrase is not here and is not in any column: it is a `vault` /
-- `passphrase` row in the encrypted `secrets` table, like every other
-- credential.
CREATE TABLE IF NOT EXISTS user_vault_config (
    user_id        TEXT PRIMARY KEY,
    vault_path     TEXT NOT NULL DEFAULT '',
    -- JSON array of service names, already filtered against
    -- `secrets_vault.eligible_services()` at the write. Filtered again on the
    -- way out, because eligibility can change under a stored row when a module
    -- is disabled.
    vault_services TEXT NOT NULL DEFAULT '[]',
    updated_at     TEXT NOT NULL DEFAULT (datetime('now')),
    updated_by     TEXT NOT NULL DEFAULT ''             -- 'web' | 'cli'
);
