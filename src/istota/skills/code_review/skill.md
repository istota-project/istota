...
A `skipped` review is not a clean review. Never report "no findings" when the review did not run. `rounds` says whether it cost anything: 0 means it was refused before any model was called, above 0 means reviewers ran and came back with nothing usable, so re-running is free only in the first case. Five more fields decide whether an `ok` is actually clean, and all five are easy to miss:
