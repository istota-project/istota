...
- A file that leaves the task is narrower still: `email --attach`, `memory_search index file` and the `health` upload/import verbs take the user's own workspace and the deferred directory, so a `Talk` attachment or a channel file has to be copied into the workspace before it can be mailed or indexed
