...
- Stdout/stderr are capped at 100 KB each in this envelope. Truncation is signalled with a trailing `\n…[truncated: N more bytes]` marker. The whole output crossed the wire; only what you are shown is trimmed.
- **There is no default timeout.** The task's own budget governs. Pass `--timeout SECONDS` when you want the command killed at a point you choose.
...
- **Stick to the documented subcommands.** There is no container engine to reach for: no Docker socket is bound into your sandbox and no `docker` binary is on your `PATH`. The verbs above are the whole surface.
