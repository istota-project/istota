...
def normalize(
    ...
    bytes do not decode as a raster in an accepted format. Nothing else escapes:
    a Pillow exception reaching a route would render as a 500 for what is an
    ordinary bad upload.
    ...
    with img:
        ...
        try:
            ...
            buf = io.BytesIO()
            # No `exif=` and no `icc_profile=`, so the output is stripped by
            # construction rather than by a scrub. That relies on Pillow's WebP
            # writer not falling back to `im.info`, which
            # `TestNormalizeStrips` asserts — if that test ever goes red the
            # answer is an explicit strip here, not a weaker assertion.
...
def put_user_avatar(
    ...
    *,
    ...
    conn.execute(
        ...
        """,
        (user_id, source, NORMALIZED_MIME, content_hash, image, remote_etag),
    )
...
def get_bot_avatar(conn: sqlite3.Connection) -> BotAvatar | None:
    row = conn.execute(
        "SELECT mime, content_hash, image, updated_at FROM bot_avatar WHERE id = 1"
    ).fetchone()
    if row is None:
        return None
    return BotAvatar(
...
def bot_avatar_hash(conn: sqlite3.Connection) -> str | None:
    row = conn.execute(
        "SELECT content_hash FROM bot_avatar WHERE id = 1"
    ).fetchone()
    if row is None:
        return None
    return row["content_hash"] or None
