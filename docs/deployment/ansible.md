...
- **WhatsApp**: `istota_whatsapp_*`; the access token, app secret and verify token belong in Ansible Vault. Per-user `whatsapp_number` and `whatsapp_bsuid` inside `istota_users` bind a user. See [WhatsApp](../features/whatsapp.md) for the Meta setup, the operating modes and the billing controls.
...
A per-user `whatsapp_number` carries the same authority for the same reason, and `whatsapp_bsuid` beside it enrolls the durable identity explicitly rather than waiting to latch it from the user's first message. Changing the number discards the 24-hour service window along with everything else learned about the previous holder, so a recycled line cannot be messaged before its new holder writes in. `whatsapp_number: ""` clears the whole binding.
