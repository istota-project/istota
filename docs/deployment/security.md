...
### A folder vault is inside the sandbox, and the passphrase is not

A user with a [folder credential vault](../configuration/credentials.md#credential-vault) configured has a KDBX file in their own workspace directory, which is bound **read-write** into that user's own task sandbox. An operator-configured absolute `vault_path` sits outside the sandbox instead.
...
So the file-level exposure is real and is exactly one thing: **a prompt-injected task can carry out the ciphertext of the whole file.** That is why `istota secret ensure --service vault --key passphrase --generate` mints the passphrase rather than documenting how to choose one. Argon2id makes a stolen KDBX useless against 256 random bits and does not make it useless against a memorable phrase, and nothing else is standing in front of that. Read the generated-passphrase rule as the boundary rather than as hygiene — and read it as bounding the *offline* case, because of what follows.

**A task can also ask for any shared credential by name, and the passphrase does not bound that.** What the vault produces is a flat namespace of names under the `vault_entries` service, and a task reaches one through the credential proxy: `istota-credential run VAR=name -- <command>` injects a value into one child process, `get` returns it to the caller. So a prompt-injected task can enumerate the namespace and read all of it. The file is the consent decision: a credential is reachable because the user shared it under a top-level `istota` group or let Istota create it under `generated/`. A skill that takes the name rather than the value keeps it out of the sandbox entirely (`browse interact --fill-credential` is the shipped one). `[security] vault_fetch_limit_per_task` — 10 by default, `0` unlimited — bounds how many fetches one task attempt may make. **A file with no top-level `istota` group is read in full**, so a user who points at a copy of their everyday password database shares all of it; the settings card and `istota secret vault-status` both say when a read was unscoped.

The proxy also accepts `vault_create`. A prompt-injected task can ask the daemon to add an entry under `generated/`, including to an operator-configured absolute path outside the sandbox. The write cannot update or delete an entry, and a produced-name collision is refused. The password is generated in the daemon and the proxy returns names and the username only; `browse interact --fill-credential` uses those names without handing the password to the task. `[security] vault_writes_per_task` defaults to three requests per attempt, including refusals; `0` disables writes. Each successful create raises a notice. A password manager holding the file open may save over the new entry later, and Istota cannot detect that open state.

Two more things are worth knowing before turning it on. Deleting or corrupting the file is a denial of service that leaves every credential in the table working and raises a notification — the sync applies nothing it cannot parse, and no service outside `vault_entries` is touched. Replacing it with an older copy the user keeps in the same tree is a genuine rollback of whatever that copy holds. Keep one copy of the vault inside the workspace.

An absolute `vault_path` removes the *file* exposure completely: the resolver refuses one that lands under the workspace, `temp_dir` or `developer.repos_dir`, so the accepted form is a host path no sandbox binds at all. It does not remove the by-name read, which is what the vault is for. It is not the default because editing the file from a phone is the feature.

**That form is yours, not the user's.** A user sets their own vault up from the settings page, and that page offers no path of any kind: their vault is a `.kdbx` in their own `istota/vault/` folder, and what they choose is a filename out of a listing the server just produced. The check an absolute path is subject to asks whether it lands outside every tree a sandbox binds, which is the right question for a path you wrote and is not a line to let anybody answer for themselves: a user-chosen absolute path would read any file the daemon can read, as the daemon, and decrypt the result into that user's own credential rows. A vault you set in `config.toml` outranks the folder, so it is not selectable from the page and the card says so; `istota user ensure --clear-vault-config --user <id>` forgets a filename a user chose.
...
Two limits are worth stating plainly. `security.sandbox_ro_paths` is bound verbatim, so an entry at or above `temp_dir` would expose the whole tree; the config loader warns on one, and `istota doctor`'s `runtime.task_control_dir` reports the same overlap for the other binds a config can produce, including a `user_resources` row — which is bounded by the workspace root and nothing else. And the deferred-op files still sit in the writable scratch directory with `O_NOFOLLOW` as their only guard against a symlink planted at a not-yet-started task's filename. Those are model-authored by design, and the scheduler's own validation of what they contain is the boundary there.
...
Threat model for manifest credentials: a compromised Claude can only request values that already exist for this user (resources are user-scoped, instance config is operator-controlled). The vault's separate create path is described above.
