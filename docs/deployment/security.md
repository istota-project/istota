...
A deployment may run project code — `npm`, `uv`, `cargo`, a test suite — inside the user's devbox rather than on the host, by setting `[developer.container] backend = "devbox"`. Off by default, and where it is off none of this exists.

**Nothing here creates a container.** The devbox is per user and standing, started by the Ansible-rendered service. The daemon starts none, and a task cannot ask for one: the one verb that recreates a container, `istota-skill devbox reset`, runs host-side in the skill CLI's own process, outside the sandbox, against the user's own container and no other.
...
**Every containment decision is made inside the container**, by the server, at the moment of use. A path is resolved with `realpath` and refused unless the result is under an explicit root list: that user's repos root, `/home/dev`, and the staging directory. The credential socket directory and the transport's own socket directory are refused by name as well. This replaces four daemon-side mechanisms that each encoded a guess about what the container's mount table looked like, which is what the two file-copy defects before it were. A process inside the container sees what the container sees.

**Everything is per user.** `[developer] repos_dir` is a per-user root on every backend: the daemon derives `{repos_dir}/{user_id}`, the container binds only that subdirectory, and the socket directory is per user too — only the per-user subdirectory is ever mounted, since the parent holds every user's socket and mounting it would be arbitrary command execution against another user's repositories. The daemon and the container also run as the same numeric uid, which is what makes a file either side writes readable, editable and removable by the other.

**The socket directory is one a container root can write to, and that is bounded rather than denied.** `dev` has sudo, so it can create files there and change their modes. What stops that being a lever is that nothing on the host ever reads anything out of that directory: the daemon connects to one socket by an absolute path it composed itself, and never opens, lists or executes anything else there. It holds one inode, is per user, is mode 0770 owned by the daemon's uid and gid, and lives on a tmpfs.
