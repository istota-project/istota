...
The URL is baked into Nextcloud at first install, so changing it later — or changing the port — means running `talk:signaling:add` again. That registration is a config write and nothing more: `--verify` is the flag for validating the server's TLS certificate, not a reachability check, so it does not matter that neither nginx nor the signaling container is up at the moment the post-installation hook runs.
