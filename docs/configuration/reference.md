...
| `room_selectable` | `[]` | Brain kinds a chat room may pin for itself, with `!brain` or the web room settings. Empty means none may, so the feature is inert until you name kinds; setting a room's brain is admin-only on top of this. A name here that is not a brain kind is warned about at load and offered to nobody. Listing a kind also widens the `doctor` checks for it — `claude` on PATH, tmux, a native API key — whether or not a room has selected it. A pinned room does not fail over: it runs the brain it names or fails saying why. |
