...
Most per-user settings are documented in [per-user configuration](per-user.md), because the DB row wins over the TOML block for nearly all of them and the CLI is the provisioning path. The two below are the exception: they exist **only** here.

| Setting | Default | Description |
|---|---|---|
| `vault_path` | `""` | A KeePass (KDBX) [credential vault](credentials.md#credential-vault) this user maintains, which the daemon reads and never writes. Empty is the feature off, which is the default for every user. A relative path resolves under that user's own workspace directory, so they can edit it from a phone. An absolute one is a host path and is refused unless it resolves outside every tree a task sandbox binds read-write — the workspace, `temp_dir` and `developer.repos_dir` — which is the form for keeping the file out of a task's reach at the cost of the phone |
| `vault_services` | `[]` | Which services that file owns. The vault is the authority for these, deletions included, so a key the table holds and the file's group does not is removed — put everything you already hold into the file before adding its service here. Empty reads the file and applies nothing, which is a usable dry run. A name a vault may not own (one whose credentials the daemon mints for itself) is dropped with a warning when the config loads |

**Neither is in `user_profiles`**, deliberately: one selects which file the daemon decrypts with a key it holds and the other selects which credentials that file may overwrite, so neither may be settable by anything downstream of a task. They live in `user_vault_config`, written by the settings page's vault form and by `istota user ensure` and by nothing else, and a row there **outranks the TOML line** — so a vault the user set for themselves survives a config regeneration, and a vault you set here is shown to them but not editable, because a row would silently shadow it. `istota user ensure --clear-vault-config --user <id>` removes a row and gives your line back. The web form takes a **relative** path only; the absolute form stays yours. The vault's passphrase is a per-user secret under the `vault` service, set from that same form or with `istota secret ensure -u <id> --service vault --key passphrase --generate`. Setting the pair here does not mean hand-editing: both shipped shapes regenerate `config.toml`, so use `istota_users` (Ansible) or `USER_VAULT_PATH` / `USER_VAULT_SERVICES` (Docker).
