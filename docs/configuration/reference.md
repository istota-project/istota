...
| `sandbox_cache_sweep_interval` | `21600` | Seconds between package-cache sweeps (ISSUE-317, 6h; 0 disables). Inert unless `[security] sandbox_cache_dir` is set, and also gated by `[security] sandbox_cache_sweep_enabled`. Moving the caches onto disk is what makes them persist, so this is what keeps the fix for a RAM leak from being a disk leak on the volume `worktree_reap_interval` is already fighting for |
...
| `sandbox_cache_dir` | `""` | Root of a disk-backed directory for the package managers' caches. Each user gets `{root}/{user_id}`, bound RW into their sandbox with `UV_CACHE_DIR`, `XDG_CACHE_HOME` and `npm_config_cache` pointed at it. Empty leaves those caches on bwrap's own root tmpfs — RAM the host cannot attribute, discarded at task exit, so every task downloads again. Put it under `[developer] repos_dir` when that is set — the Ansible role derives it as `{repos_dir}/.package-caches` and leaves it empty when `repos_dir` is. uv populates a venv by hardlinking out of its cache, and `link(2)` compares *mounts* rather than devices, so a cache root anywhere else returns `EXDEV` even on the same filesystem and every worktree pays for a full copy; the repos bind covering the cache bind is what puts both on one mount. That covering also makes every user's cache directory reachable read-write from an admin developer task, which the sandbox closes by masking the sibling directories with empty read-only tmpfs mounts after both binds (ISSUE-319). Here the mask *is* the boundary rather than defence behind the skill CLIs, so a covered cache is refused outright on a bwrap without `--disable-userns`; when the root cannot be listed, holds a symlink, holds an entry that cannot be classified, or holds more than a few hundred entries. Every refusal drops the bind *and* the environment variables together, since the covering bind means an unbound cache path is still reachable. On NativeBrain, whose file tools run with no namespace, the siblings become write denials instead — which closes the planting path and leaves them readable. One residual, stated: the root itself stays writable inside the sandbox, so a task can pre-create a cache directory for a user who has none yet, and the daemon adopts it. Ignored with one warning when relative, missing, unwritable, under a database directory, or at or above a path the sandbox already mounts |
| `sandbox_cache_sweep_enabled` | `true` | Bound what `sandbox_cache_dir` grows to (`src/istota/sandbox_cache_sweeper.py`). Every sweep runs the package managers' own cheap reclaim (`uv cache prune`, `npm cache verify`); a per-user cache still over its ceiling afterwards is wiped with their `clean` verbs. The sweeper deletes no file itself — a tool that is missing, that fails or that times out is reported and the cache is left alone. A user with a task in flight is skipped entirely, and `--force` is never passed to uv, so its own in-use check still stands. Runs on `[scheduler] sandbox_cache_sweep_interval`; inert while `sandbox_cache_dir` is empty |
| `sandbox_cache_max_gb` | `10.0` | The ceiling, per user, over the whole `{root}/{user_id}` directory. The Ansible role sets 4.0 instead, sized against the reference deployment: one `uv sync --all-extras` writes about 1.8 GB, so the ceiling has to clear one resolution, and the root now shares a 40 GB volume with the worktrees. A size budget rather than an age window because one `uv sync --all-extras` writes about 1.8 GB in a single command, which blows any sane window at once. Clamped to a 1 GiB floor: below that the ceiling is under one dependency resolution's working set, so every sweep would wipe a cache that is doing its job. `XDG_CACHE_HOME` points at the user root, so a third tool's cache counts toward the budget while neither reclaim verb can touch it — that case is reported with the largest remaining subdirectory named, not deleted by hand |
