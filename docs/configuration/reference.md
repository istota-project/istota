...
**Neither is in `user_profiles` and neither has a web route**, deliberately: one selects which file the daemon decrypts with a key it holds and the other selects which credentials that file may overwrite, so neither may be settable by anything downstream of a task. The vault's passphrase is a per-user secret under the `vault` service and is provisioned with `istota secret ensure -u <id> --service vault --key passphrase --generate`. TOML-only does not mean hand-edited: both shipped shapes regenerate `config.toml`, so set the pair through `istota_users` (Ansible) or `USER_VAULT_PATH` / `USER_VAULT_SERVICES` (Docker).
