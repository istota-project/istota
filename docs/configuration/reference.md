...
| `vault_sync_interval` | `300` | Seconds between [credential-vault](credentials.md#credential-vault) syncs (0 disables, and disables the start-up pass with it). Inert unless some user has a `vault_path`. Start-up alone is not enough — a user edits their vault at 3pm and nothing happens until the next restart — and five minutes is chosen against the rclone dir-cache lag, which adds its own delay on top; a shorter interval mostly buys more `stat` calls. A cycle whose file hash is unchanged stops there, so the steady-state cost is one bounded read per user rather than a key derivation |
...
| `skill_proxy_timeout` | `300` | Proxy command timeout (seconds). Capped at `skill_client_wait_seconds` minus 30; `[security.skill_proxy_timeouts]` gives one skill a different budget under the same cap |
| `skill_client_wait_seconds` | `600` | How long the sandboxed `istota-skill` client waits on the proxy socket. The bound every skill budget sits under — the global and every per-skill entry are capped at this minus a 30s margin, so a skill wanting more than 570s needs this raised too. Exported to the client as `ISTOTA_SKILL_CLIENT_WAIT`; the proxy derives its ceiling from this field, never from the export, which lives in the model's environment |
| `vault_fetch_limit_per_task` | `10` | How many shared-credential fetches (the `vault_entries` namespace a user's [KDBX vault](credentials.md#credential-vault) produces) one task *attempt* may make over the proxy socket. `0` is unlimited and is the only spelling of unlimited — an absent key is this default, never "off". Enforced by the proxy rather than by the `istota-credential` shim the model calls, since that program sits in a directory the model can overwrite. Counted per request whether or not the name resolves, so probing for absent names is not free; listing the namespace is not counted |
...
Most per-user settings are documented in [per-user configuration](per-user.md), because the DB row wins over the TOML block for nearly all of them and the CLI is the provisioning path. The one below is the exception: it exists **only** here.

| Setting | Default | Description |
|---|---|---|
| `vault_path` | `""` | A KeePass (KDBX) [credential vault](credentials.md#credential-vault) this user maintains, which the daemon reads and never writes. Empty — the default — means their own `istota/vault/` folder decides, which is the ordinary route and needs no line here at all. A relative path resolves under that user's own workspace directory, so they can edit it from a phone. An absolute one is a host path and is refused unless it resolves outside every tree a task sandbox binds read-write — the workspace, `temp_dir` and `developer.repos_dir` — which is the form for keeping the file out of a task's reach at the cost of the phone |

**It is not in `user_profiles`**, deliberately: it selects which file the daemon decrypts with a key it holds, so it may not be settable by anything downstream of a task. What a user sets for themselves is a *filename* out of their own vault folder, validated by membership in a listing the server just produced and stored in a reserved KV namespace no task may write. This line **outranks that choice**, so a vault you set here is shown to them but not selectable, because a stored choice your line overrides is a control that does nothing; `istota user ensure --clear-vault-config --user <id>` forgets a choice they made and returns them to the folder's own rules. The vault's passphrase is a per-user secret under the `vault` service, set from the settings card or with `istota secret ensure -u <id> --service vault --key passphrase --generate`, and without one nothing is read whichever route named the file. Setting the path here does not mean hand-editing: both shipped shapes regenerate `config.toml`, so use `istota_users` (Ansible) or `USER_VAULT_PATH` (Docker).
