...
| `max_avatar_kb` | `4096` | Byte cap on a profile-picture upload, in KB. Enforced on the declared `Content-Length` before the body is read and again on the running total. `0` switches uploads off — the endpoint answers 503, and a stored picture is still served. Separate from nginx's `client_max_body_size`, which bounds what reaches the process at all and is sized for chat attachments |
