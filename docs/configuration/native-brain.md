...
- **Task timeout.** The native loop runs under a wall-clock deadline of `scheduler.task_timeout_minutes` (`istota_scheduler_task_timeout_minutes`, 60 on an Ansible deploy; 30 is the in-code default). On expiry it signals abort (killing any in-flight bash subprocess at the next poll), waits a short grace, then hard-cancels, and returns `stop_reason="timeout"`. This matches `claude_code` and prevents a runaway loop from outliving the scheduler's stuck-task reclaim (which would otherwise double-execute the task). `max_turns` is a second, coarser backstop, and it takes the same split: `istota_brain_native_max_turns` is **200** on an Ansible deploy against the in-code default of 100, because the cap is a loop backstop rather than a budget and 100 was low enough to be reached by ordinary long work rather than only by a runaway.
