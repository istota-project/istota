...
**Another client using the same session.** If a copy of the session directory is started somewhere else, WhatsApp closes one connection each time the other logs in (status 440 in `sidecar.log`). The sidecar waits 30 seconds after each such close instead of reconnecting at once. After five in ten minutes it stops reconnecting, refuses sends, and the admins get one alert naming the cause. It then tries again after 15 minutes, and after an hour twice more. A try that stays connected for five minutes ends the outage. If the other client is still there after the last try, the sidecar gives up, a second alert says to re-pair from Admin, Connections, and `istota doctor --only whatsapp.baileys_bridge` fails with the same instruction. If you do not know what the other client is, remove the old device under Linked Devices on the phone before re-pairing: a re-pair creates a new device and does not revoke the copied credential. The run is kept in `connection-replaced.json` inside the session directory, so restarting the sidecar does not make it reconnect early. A `creds.json` changed by anything other than the sidecar ends the run.
