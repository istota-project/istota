...
### Declared points while you sit still

A *declared* point is a coordinate the iOS shell asserts while the device is on the configured home SSID rather than a fix it took, marked by `location_pings.wifi_zone` (ISSUE-229). It is constant by construction, so re-sending it says nothing the first one did not — and the shell sent one every 60 s for as long as the device stayed put. Measured over a month, that was 26,713 of 39,319 native pings at a single coordinate, one stay running 42 hours.

The receiver now drops a declared point that adds nothing: the stay is already established by the newest three stored rows, every one of them is declared and at this same coordinate, and the newest is under 15 minutes old. Any of those failing means something happened worth recording, and an out-of-order or unparseable point is always stored. A suppressed point returns before the insert, so the place state machine does not re-run on a place it is already sitting on; visits are unaffected, since a stay is defined by when it started and by where you turn up next (ISSUE-349). The shell stops emitting the repeats in its next build; until then the server drops them on arrival.
...
It authenticates through the shared Garmin connection (Settings → Connected services; the same credentials the health module uses), so connect Garmin once and both features work. The dedup is spatiotemporal: a Garmin point is dropped only when a native ping exists within both a time band (`--guard-band`, default 300 s) and a distance band (`--guard-radius`, default 150 m) of it — so a phone left at home never shadows a run happening elsewhere. **Declared points (`wifi_zone = 1`) are not counted as coverage at all**: those are a coordinate the phone asserts while it is on the home SSID, not a fix it took, so they record nothing to shadow with. Left in, they deleted real track: a run starting and ending at the door passes within `--guard-radius` of the parked phone twice, and a measured 22-minute run lost its first 151 m and its last 171 m that way (ISSUE-348). A phone that is genuinely stationary and taking real fixes is the same trap one step narrower and is deliberately not handled — those rows are at least measurements. Imported points are placeless breadcrumbs (`place_id` NULL) that show on the map and in history but don't create place visits. Re-running is idempotent (evict-then-reinsert per activity), and a late Overland upload for a gap-filled window evicts the now-covered imports on the next run.
