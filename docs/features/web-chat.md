...
**You are told, once, where it matters.** A warning alert under the app bar reads "Offline — messages will send when you’re back" for as long as the app cannot reach your server. It is not a toast: connectivity is a state rather than an event, and taking the explanation away after four seconds while the composer still cannot reach anything is worse than saying nothing. It sits in the page's own band rather than on the composer, because it is a statement about the app rather than a caption on the text box — and so it also stands in the All, Unread and Starred views, which have no composer and cannot be read offline either. What decides it is what requests actually did, not what the browser thinks of its network interface — a captive portal that answers every request with its own login page reads as offline, because your server was never reached.
