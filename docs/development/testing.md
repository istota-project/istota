...
Four properties are worth knowing. The first three are traps if you invoke testmon by hand; the fourth is a rule for writing tests:
...
**A test that reads source instead of running it needs help to be seen.** A drift guard asserting against `inspect.getsource(...)` — the shape that proves a deleted copy has not regrown — executes none of the lines it reads, so testmon records no dependency on them. Measured: changing `location_db.connect` selected 26 of `tests/test_location_module.py`'s 39 tests and left out the one test in the file whose whole subject is that function. About a dozen guards of this shape arrived with the duplicate code consolidation, and `qt` could not see any of them (ISSUE-459). They read through `tests/support/drift.py`'s `source_of` now, which records the line range it read against the running test; `drift.install()`, called from `tests/conftest.py`, merges those ranges into the coverage testmon is about to fingerprint, so the guard depends on the block it read — plus the module's own structure block, as any other dependency does. The same change selects 27 tests now. `tests/test_drift_selection.py` measures both directions in a throwaway project, and fails on a raw `inspect.getsource` anywhere under `tests/`. A guard that reads a file's *text* off disk instead — the `rglob("*.py")` sweeps over `src/` — is still invisible and is not covered by this.
...
It costs no stack, because `storage.py` branches on `has_workspace` rather than on the backend and `render-config.sh` writes `workspace_path` as the literal `/mnt/shared` on every profile. Briefings, memory and the tasks file take the identical path under both, while `nextcloud_mount_path` stays unset because the Docker volume is not a FUSE mount. Two things differ, both pure functions of a `Config`: the prompt's file-tool vocabulary and the skill menu (`available_capabilities()` drops `nextcloud` on an empty URL). Both are prompt content and live in the goldens as the `base_nextcloud` / `base_local` pair; whether `NC_URL=""` renders a config that loads as `local` at all is in `tests/test_render_config.py`.
...
One product gap is held here by a named test rather than fixed, so a fix arrives as a reviewed golden diff and turns it red instead of passing silently: `custom_system_prompt` cannot change the assembled prompt at all, because it is read at brain-request assembly well past the `dry_run` return — not a defect, since that is the brain's system prompt rather than the task prompt, but the identity is asserted so that a change routing it into the task prompt is visible.

The gap that used to sit beside it is closed. `format_cli_skills` applied neither the capability gate nor the effective disabled set, so a Nextcloud-free install was told `istota-skill nextcloud` exists in the same prompt that omitted the skill from the on-demand menu; the test holding that shape went red on the fix, which is what it was for. Both producers read one derivation now, and the `Room:` line names `istota-skill rooms list` only where that skill is enabled. It applies the same *disabled set* as the menu rather than the menu's whole gate — the experimental and dependency gates are not folded in, which `advertised_cli_skills`' docstring records.

If you add a golden case, note that the synthetic catalogue carries a `rooms` skill deliberately. The `Room:` line names that CLI and emits the clause only where `rooms` is in the effective index, so a catalogue without it would make every `room_*` golden snapshot the gated form and quietly stop covering the ungated sentence. The gated form is a unit case in `tests/test_prompt_room_identity.py` instead.
