...
**The devbox half of the `integration` marker needs a running devbox whose exec server answers.** `tests/test_skills_devbox_integration.py` is the only tier that reaches a real container: every other devbox test drives the protocol against a server in a tmpdir on the host, which proves the skill speaks the wire and not that the wire reaches a container. The assertions that make it worth running are the ones only a container can satisfy — a hostname that is not the host's, and a path that exists in the image and nowhere else. Run it as `ISTOTA_USER_ID=<user> uv run pytest -m integration tests/test_skills_devbox_integration.py -n0`; without `ISTOTA_USER_ID`, a configured `exec_socket_dir` and a transport that answers a `ping`, every test skips, so the file is inert on a laptop. **Run it from a host shell still**, though for a different reason than before: this CLI resolves its socket from the config file, and the config directory is bound into no sandbox, so from inside a task it loads defaults, reads `backend = none` and skips. What changed is that the skip is honest. It used to spend a third of its length probing for a Docker-API allowlist refusal and skipping on it: the socket a sandboxed task saw was `istota-docker-proxy`, which tracked the exec ids it issued and denied a raw `docker exec` at the exec-inspect step — *after* the command had run — so an envelope reading `ok` / `1` said nothing about the container (ISSUE-313). That proxy is retired, the skill runs `docker` for nothing but `reset`, and the status now comes from `waitpid` over the protocol, so there is no refusal to mistake for an answer and nothing to probe for.
