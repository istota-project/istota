...
Every model ID in the codebase resolves through the active brain. Effort is an orthogonal **`:effort` modifier** on any reference — `split_effort(raw)` (in `brain/_aliases.py`) peels a trailing `:<effort>` ∈ `low|medium|high|xhigh|max` (`opus:high`, `smart:low`, `opus:xhigh`); every brain's resolver applies it first. Then two layers, top to bottom:
