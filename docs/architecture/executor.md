...
1. **Header**: role definition, user_id, current datetime, task_id, conversation_token, source, output target, privileges, and a line stating the database is reachable only through skill CLIs — the path itself is deliberately not in the prompt
