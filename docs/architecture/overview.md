...
So how does a "dumb" schedule reach the agent? Through the **deferred subtask** mechanism. All three task paths run with a deferred directory set — the sandboxed **Brain** path (bwrap masks the database directories out entirely, so the model can neither read nor write the DB directly and defers instead), the **skill** path, and the raw shell **command** path, which reuse the same rail — and write JSON op files into `ISTOTA_DEFERRED_DIR`, which the unsandboxed scheduler applies *after* the task succeeds. A command row was the odd one out until ISSUE-233: it got no deferred dir, so a skill CLI it invoked wrote directly where it could and silently wrote nothing where it could not. Two consequences of putting it on the rail are worth knowing before you write one. A write is now applied only if the command as a whole exits 0 — `_purge_deferred_files_for_retry` discards the ops of a failed attempt, so `istota-skill kv set … ; something-that-fails` no longer persists the value. And a write is not visible to a later read *within the same command*, because reads still go straight to the database. Subtask creation is the reason to accept both: it is the only route from a deterministic schedule to an agent, and command rows are admin-only. One of those file types is a subtask request:
...
**Deferred writes for sandbox compatibility.** With bubblewrap masking the database directories out of the sandbox, skills write JSON files to a writable temp dir. The scheduler processes these after task completion. (A skill CLI invoked through the credential proxy runs host-side and *could* write directly; it still defers, so the write lands only if the task succeeds.)
