...
`workspace_path`, so `/Users/alice` on disk is `/Users/alice` over DAV. On
...
### `WhatsAppConfig`, `WhatsAppCloudConfig`, `WhatsAppBaileysConfig`, `WhatsAppTemplateConfig`

`[whatsapp]` is one business phone number reached through one adapter, and only three fields are at that level: `enabled`, `provider` (`baileys` | `whatsapp_cloud`, defaulting to `baileys`) and `business_phone_number` — the dedicated line is the same line whichever adapter holds it, which is what lets a deployment switch without the conversation token, the bindings or the ledger moving. Each adapter's own settings are its nested block, the arrangement `[sms]` has with `twilio` and `telnyx`. `[whatsapp.cloud]` is everything Meta's Cloud API needs: the two decimal ids `waba_id` and `phone_number_id`, the three credentials, `graph_api_version`, `business_timezone`, `request_timeout_seconds`, `billing_policy`, `monthly_service_attempt_limit`, and `[whatsapp.cloud.proactive_template]` (`enabled`, `name`, `language`). `[whatsapp.baileys]` is `session_dir` and `library_version`, both resolving themselves when empty; neither is a credential, because a paired WhatsApp Web session's credential is on disk.

**A pre-adapter flat `[whatsapp]` still loads, and the compatibility is two separate decisions.** `_migrate_whatsapp_flat` runs before the dataclass walk (through `normalize_legacy_document`, which the two generator drift guards call for the same reason) and rewrites every moved key into `[whatsapp.cloud]` unconditionally, dropping the flat spelling so it cannot land in the unknown-key report; a nested value for the same key wins. Separately, a flat block carrying a non-empty `waba_id`, `phone_number_id` or one of the three credentials and **no** `provider` key selects `whatsapp_cloud` — which is what keeps an existing Cloud deployment serving `/webhooks/whatsapp` across the default flip. **Non-empty values, never key presence**: both shipped generators render the whole block unconditionally — every key at its default, `waba_id = ""` and the rest — so a presence test would read every deployment as a Cloud one, including the Baileys ones. That was true of the flat spelling and is equally true of the nested one, which is why the signal is read against the **resolved** `[whatsapp.cloud]` table rather than against the flat keys: moving the values is a spelling, and their being Meta identifiers is the fact. Both generators now write the nested shape and render `provider` only when the operator names one, so the flat spelling survives only in hand-written files; `config.example.toml` carries the deprecation. There is still deliberately no deprecation warning, and the reason is now only the cost — `load_config` runs per host-side skill CLI call, so a per-process line is a per-call line there.

**Validation is split in two, and the split is the ISSUE-058 rule.** `whatsapp_structural_config_errors` fails the load, and only the *active* adapter's arm runs: the provider name is always checked, then `_WHATSAPP_PROVIDER_VALIDATORS` dispatches. Under `whatsapp_cloud` that is what it always was — the ids decimal, the number exact E.164, the timezone a real IANA zone, the timeout 1 to 30, the policy one of `free_guard` / `allow_paid`, the cap 1 to 1000 under `free_guard`, a template valid only under `allow_paid` with a name and a language. Under `baileys` there is nothing a deployment is required to write, so only a `business_phone_number` that is set is checked for shape. `whatsapp_credential_errors` does not fail the load, because the Ansible role renders `access_token`, `app_secret` and `verify_token` empty on purpose under `istota_use_environment_file` and delivers them through `secrets.env`, which the role's own `command:`/`script:` tasks cannot read — so raising there fails the play on a config the daemon loads fine. It reports the **active provider's** missing fields rather than Meta's three unconditionally: `outbound._gate` records `unconfigured` for any non-empty result, so the unconditional form refused every Baileys send for want of a token that adapter does not use. Doctor reports them and the transport refuses at send time. `whatsapp_config_errors` returns both. The three `ISTOTA_WHATSAPP_*` credential env overrides keep their names and point at `whatsapp.cloud`, so an existing `secrets.env` is unchanged.

`graph_api_version` accepts `v25` and `v25.0`; empty follows the version the installed PyWa release pins. `business_timezone` is what the monthly cap's calendar month is computed in, so changing it moves a month boundary. `whatsapp_webhooks_enabled` is `enabled` plus the active `provider` being one that receives over an HTTP callback at all (`WHATSAPP_WEBHOOK_PROVIDERS`), and has no callback-only arm, unlike its SMS sibling. See `.claude/rules/whatsapp.md` and `docs/features/whatsapp.md`.
...
**The protection checks run against the cache's parent**, on both branches, which can only ever refuse more. One consequence has no escape hatch any more and is worth stating plainly: `_validate_workspace_dir` overlaps in *both* directions, so a `developer.repos_dir` overlapping the source tree, the workspace root, a database directory or a `$HOME` dotfile directory loses its disk cache on every task — and `sandbox_cache_dir` cannot be used to put it somewhere else, because that branch is not taken. The refusal is a fact about `repos_dir`, and the fix is to move `repos_dir`.
...
```
Properties / methods:
- `has_workspace`: `bool` — True if `workspace_path` is set
...
- `workspace_root(user_id=None) -> Path | None`: root of the local workspace tree (`None` when no local tree exists). Scoped to `{workspace}/Users/{user_id}` when `user_id` is given, else the bare workspace root. De-dups the `workspace / "Users" / uid` idiom inlined across the codebase — not a storage abstraction (no I/O, no backend switch).
Methods:
