...
| `build_allowed_tools(is_admin, skill_names)` | Returns `["Read", "Write", "Edit", "Grep", "Glob", "Bash", "WebSearch"]`, plus `"WebFetch"` **for an admin only** — the one thing `is_admin` decides here. The native brain builds its daemon-side `WebFetch` from this list and that tool reaches the network outside the CONNECT allowlist, where the same user under a CLI brain has `--unshare-net` plus the allowlist; scoped unconditionally rather than only for a room-pinned brain, so a native-default deployment is covered too. `build_prompt`'s Tools section names the tool under the same flag. For ClaudeCodeBrain / TmuxClaudeBrain the *list contents* no longer reach the CLI — both run with `--dangerously-skip-permissions` (no `--allowedTools` allowlist), so the model gets its full default toolset and the bwrap sandbox + network proxy + clean env are the boundary. The list survives as (a) NativeBrain's in-process tool filter and (b) the non-empty/empty signal distinguishing a tool-bearing task from a text-only one. `Agent` + `Workflow` (the harness's multi-agent fan-out) stay denied via `--disallowedTools`. |
