...
**One enriched string, five consumers.** `effective_prompt` is the typed request plus any audio transcript plus the rendered OCR section, and it goes to `select_skills`, to `build_prompt` and — as `retrieval_query` — to memory recall, playbook recall and the knowledge-graph filter. An attachment is deliberately supplied input, so its text earns recall the way any other input does; the audio transcript already reached those passes.
