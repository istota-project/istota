...
- `_extra_ro_binds` carries the file into `build_bwrap_cmd`, whose `extra_ro_binds` are applied after the read-write bind of `user_temp_dir`, so the later read-only bind shadows the earlier one. This is that parameter's first production consumer. It covers every bwrap-wrapped child, including the native `Bash` tool.
