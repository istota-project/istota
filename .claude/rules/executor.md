...
| Developer | `DEVELOPER_REPOS_DIR` | `config.developer.repos_dir` **plus the task's user id** (if enabled). The manifest `EnvSpec` is `from: "config_per_user"`, a source that exists for this one job: an `EnvSpec` cannot interpolate a user id, and left global this var stayed `code_review`'s containment root, so `code_review run --worktree {repos_dir}/other-user/…` remained in bounds — the cross-user reach the per-user root closes, surviving in the one consumer whose argument the model picks. Both manifests (`developer/skill.md`, `code_review/skill.md`) use it. It resolves to nothing where there is no user id, never to the unscoped root. |
...
| `resolve_sandbox_cache_dir(config, user_id)` | `{security.sandbox_cache_dir}/{user_id}`, created, or `None`. One predicate for the RW bind, the cache environment in `execute_task`, and `native_fs_roots`. Per user because uv trusts its unpacked wheels on read, so a shared cache is a cross-user code path. Returned **as written**, not resolved, so a cache under a symlinked `repos_dir` lands on the same mount the repos bind does — otherwise `link(2)` returns EXDEV and every worktree pays for a full copy, silently. **Never raises**; every rejection falls open. Rejects a relative path, a non-writable directory, anything under a database directory (checked here, since `_validate_workspace_dir` skips a relative `db_path`), the rest of `_validate_workspace_dir`'s blocklist, and anything at or above a path the sandbox already mounts — see `_sandbox_bind_targets`. Warns once per process per distinct refusal. |\n| `_sandbox_bind_targets(config)` | What `build_bwrap_cmd` mounts, that a cache must not be mounted *above*. bwrap applies argv in order, so the late cache bind would cover an earlier mount whose destination is beneath it: `$HOME/.cache` over the read-only huggingface bind, `config.temp_dir` over every workspace and the `.developer` credential helpers, `$HOME/.local` over the `claude` binary, `developer.repos_dir` over the admin gate. `_mask_protected` solves the same problem for the masks; this is its counterpart. Equal-or-ancestor, not overlap — a cache *inside* one of these is the documented shape. |
...
| `build_bwrap_cmd()` | Build bubblewrap sandbox command wrapper. Binds `resolve_sandbox_cache_dir(config, task.user_id)` RW when set, before the database masks and gated on neither admin nor the developer skill — any task running a package manager writes a cache, and without the bind that write lands on bwrap's root tmpfs (ISSUE-305). Binds the repos tree at `{developer.repos_dir}/{task.user_id}` — a per-user root, so one user's task cannot see another's worktrees. Binds the per-user exec socket **directory** `{[developer.container] exec_socket_dir}/{user_id}` RW when the backend is `devbox` and `"developer" in authorized_skills` — byte for byte the predicate `_build_network_allowlist` already uses for the package registries, so the exec socket is bound exactly where the registries are allowed. The directory rather than the socket file, because a server restart unlinks and recreates the inode; the per-user subdirectory rather than the parent, because the parent holds every user's socket. Binds **no** Docker socket and no `docker` CLI: the allowlist proxy and its bind are both gone, and unlike that proxy this bind cannot be ungated — an allowlist is safe to hand every task, an arbitrary-command channel into a permissive-egress container is not. `selected_skills` was a dead parameter and is now `authorized_skills`, which is the set that decides the bind. Also ro-binds `custom_system_prompt_path(config)` — the file, never its directory. |
