...
**No database is reachable from the sandbox.** `build_bwrap_cmd` ends with `--tmpfs` masks over `config.db_path.parent` and `config.module_db_root()` — the **last** mount operations, since bwrap applies argv in order. Nothing binds the framework DB or its `-wal`/`-shm` any more (the admin `--ro-bind` and `security.sandbox_admin_db_write` are both gone), and `native_fs_roots` dropped its matching admin read root.
