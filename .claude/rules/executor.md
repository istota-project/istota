...
| Core | `ISTOTA_SANDBOXED` | `"1"` when `sandbox_enabled and _bwrap_available()`. Sandbox env only (added *after* the proxy's base env is snapshotted). `skill_client._run_direct` refuses when it is set. |
...
| Devbox | `ISTOTA_DEVBOX_CONTAINER`, `ISTOTA_DEVBOX_DOCKER_CLI`, `ISTOTA_DEVBOX_EXEC_TIMEOUT`, `ISTOTA_DEVBOX_MAX_OUTPUT_BYTES` | `config.devbox.*` (set unconditionally when `config.devbox.enabled`, no selection gate). Container name defaults to `f"{container_prefix}{task.user_id}"`. `build_bwrap_cmd` `--ro-bind`s `docker_cli` and binds the per-user **Docker-API allowlist proxy** socket (`{api_proxy_socket_dir}/{user_id}.sock`) at the conventional in-sandbox path `/var/run/docker.sock` whenever `config.devbox.enabled and config.devbox.api_proxy_enabled` and the proxy socket exists — **no `"devbox" in selected_skills` gate, and the raw root-equivalent socket is never bound**. The devbox CLI is unchanged (connects to the default path); the proxy (`src/istota/docker_proxy.py`) forwards only the allowlisted ops on the user's own container, so binding it unconditionally is safe even for untrusted-content tasks. `config.devbox.docker_socket` is deliberately **not** exported into the task environment (ISSUE-284): nothing read it, and the field carries two meanings — the *real* root-equivalent socket that `docker_proxy` connects to upstream, and the in-sandbox *mount point* `build_bwrap_cmd` binds the proxy at. A name in the model's own environment invites a later reader to treat it as the socket it may use. |
