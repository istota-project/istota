...
**Two residuals, stated rather than implied away.** `user_temp_dir` is per *user*, not per task, so both guards name one exact path and neither expresses `task_*_system_prompt.txt` as a shape: a second concurrent task of the same user has the directory in its write roots and can overwrite *another* task's system prompt file. Closing that means moving the artifact into a denied subdirectory or teaching `ToolEnv` and the tool-server protocol a pattern, neither of which is in scope here. The `O_NOFOLLOW` open closes the other half — a dangling symlink planted at a not-yet-started task's filename, which a plain `write_text` would follow into an arbitrary write as the daemon user. The same symlink exposure has always existed on `task_{id}_prompt.txt` and on every deferred-op file; it is unchanged, not newly introduced, and unfixed.
