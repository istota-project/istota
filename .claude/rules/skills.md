...
**Subcommands**: `list`, `check`, `balances`, `query`, `report`, `lots`, `wash-sales`, `add-transaction`, `edit-transaction`, `backfill-ids`, `sync-monarch`, `debug-monarch`, `import-csv`, `invoice` (sub: `generate`, `list`, `paid`, `create`, `void`), `work` (sub: `list`, `add`, `update`, `remove`, `backfill-ids`)
...
**Invoicing config (clients / entities / services)** is editable from the browser as well as the CLI: the Clients tab and the money settings page drive the `/config/*` routes that already backed `istota money client|company|service`. Validation is split by who it protects against. The **invariants whose violation changes behaviour silently** live in `config_store` and raise `ValueError`, so the CLI and the agent are held to them too: `service.type` ∈ `hours|days|flat|other` (an unknown type has no branch in `entry_line_item` and quietly bills as hours), `client.schedule` ∈ `on-demand|monthly` (`check_scheduled_invoices` only acts on `monthly`), a finite non-negative `rate`, integer day ranges, `terms` as `int | str`, beancount account/commodity shapes, and a slug-shaped **new** key (`^[A-Za-z0-9][A-Za-z0-9_-]{0,63}$`, checked only when the row doesn't exist, so an existing legacy key stays editable). Only the fields a caller actually passes are checked — validating the merged record would make a non-conforming row uneditable. The **shape** checks (`_coerce_{client,entity,service}_fields` in `routes.py`, modelled on `_coerce_work_fields`) stay route-side: unknown keys rejected by name, JSON types checked with the `isinstance(value, bool)` guard, control characters refused via `_CONTROL_CHARS_RE` (which lets `\n` through, so multi-line `address` / `payment_instructions` work). The route maps the store's `ValueError` to a 400 with the message intact.

Three route-level guarantees the forms depend on: `POST` **creates** (409 on a taken key, rather than the silent upsert it used to be) while `PUT` keeps upsert semantics for `ensure`-style callers; `DELETE` 404s on a missing record; and each `DELETE` resolves inbound references first, with a strictness matching how badly the absence corrupts things — a **service** named by any work entry is refused (deletion unbills future work *and* re-renders past invoices short, since `GET /invoices` rebuilds totals from live config), an **entity** is refused while anything depends on it — a client naming it, being the stored `default_entity`, or being the *effective* default that blank-entity clients fall back to (the fallback silently reassigns the invoice to another legal entity) — and a **client** is allowed, returning its `work_entries` count so the confirm dialog can say what it costs. The entity guard reads the *stored* `default_entity` scalar (`config_store.get_invoicing_setting`), not the one `load_invoicing` derives, or a fresh user's only entity would be permanently undeletable. Keys are immutable — they are referenced by plain string from four places with no foreign keys and no transaction spanning the money DB and the work TOML files — so the forms take a key on create and render it as static text thereafter. Reading is split for the same class of reason as the work form's omit-vs-null rule: the Clients page loads `GET /clients` (defaults resolved in, drives the cards) *and* `GET /config/clients` (raw, drives the form), because binding an edit form to the resolved shape materialises the default onto the record and silently stops a later `default_entity` change from propagating. Forms send `""` to clear an optional field (the store skips `None`) and omit `bundles` / `separate` entirely (the merge preserves them, which is what lets them ship without a nested-list editor). Spec: `Specs/Done/money-config-editing-clients-entities-services.md`.
