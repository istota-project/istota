...
Three placement decisions that look arbitrary and are not. **The gate is in the CLI verbs, not in `send_email` / `reply_to_email`**: those library functions also carry briefing delivery and `outbound_drafts.release`, so gating them would hold every briefing and re-hold every approval the user had just granted. **It is not in the skill proxy** (`skill_proxy.py`), which would need per-skill argv parsing to find recipients. **It is not in the scheduler**, which sees the task only after SMTP has run. `output` is exempt because its recipient is the inbound correspondent, who already cleared the inbound gate — the one hole there is `thread_match`, i.e. ISSUE-234, seen from the outbound side.
