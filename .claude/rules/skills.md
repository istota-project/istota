...
**Subcommands**: `log`, `stats`, `latest`, `panels`, `panel`, `add-panel`, `add-biomarker`, `trend`, `upload`, `summary`, `settings`, `set`, plus `garmin-status` / `garmin-sync` / `garmin-disconnect`. Documents (health-document-attachments spec): `documents [--entity TYPE:ID]`, `document <id>`, `attach-document --path P --to TYPE:ID`, `detach-document <id> --from TYPE:ID`.
