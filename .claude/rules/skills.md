...
**Subcommands**: `list`, `check`, `balances`, `query`, `report`, `lots`, `wash-sales`, `add-transaction`, `edit-transaction`, `backfill-ids`, `sync-monarch`, `debug-monarch`, `import-csv`, `invoice` (sub: `generate`, `list`, `paid`, `create`, `void`), `work` (sub: `list`, `add`, `update`, `remove`, `backfill-ids`), `portfolio` (sub: `import`, `snapshots`, `summary`, `history`, `diff`, `symbol`, `delete-snapshot`, `accounts`, `classify`, `unclassify`, `autoclass`)
