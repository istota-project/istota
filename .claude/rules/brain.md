...
- **File-tool confinement (NB-1).** The in-process file tools run outside bwrap,
  ...
  express a hole inside a root. Up to two entries: `{user_temp_dir}/task_<id>_system_prompt.txt`, Istota's own
  standing instructions, which a `Write` would otherwise rewrite under the
  running task, on every shape; and `{user_temp_dir}/.developer`, which holds
