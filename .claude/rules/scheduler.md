...
| Host pressure | `_emit_host_pressure_breadcrumb()` → `host_pressure.read_sample` + `read_tmpfs_usage` + `breadcrumb` (daemon-only) | `host_pressure_breadcrumb_interval_seconds` | — (logs only) |
