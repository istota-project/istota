...
6. **Failure path**. Both the branch that writes the row and the terminal-event block below it classify the finished attempt through **one** pair of functions, `retry_flags(task, result, *, success)` and `decide_retry(task, result, *, six flags, success=False)` (F4). `retry_flags` returns a dict keyed by `decide_retry`'s own keyword names, so a site cannot derive five of the six and quietly disagree about the sixth — which is what happened: the event block omitted `is_sigpipe`, so a command task killed by SIGPIPE was failed permanently by the branch while the block told a watching client it would be retried and skipped the terminal frame. **That was latent, never live**: `is_sigpipe` needs `task.command`, and `process_one_task` builds an `EventWriter` only on the brain path, so the block holding the wrong answer never ran for the one task shape that could produce it. `RetryDecision.reason` names the winning arm (`cancelled` / `policy` / `requeued` / `oom` / `permanent` / `sigpipe` / `attempts_exhausted` / `retry` / `success`) and is **diagnostic — nothing in `src/` selects on it.** Both sites branch on the `retry_flags` dict where they need a specific class and on `decision.will_retry` for the rest, which is what keeps each arm's condition identical to the one it replaced. Do not refactor them onto `reason`: four reasons have to collapse onto the row branch's single else-arm, and the event block tests `is_requeued` without having excluded `is_cancelled` first. `decide_retry` owns the `1 << (attempt_count * 2)` backoff and the `attempt_count < max_attempts - 1` budget test, each written once; `tests/test_scheduler_retry_decision.py` enumerates all 64 flag combinations against the branch's pre-change rule and greps the module for a second copy. `run_task_inline` (the REPL path, no retry ladder by contract) deliberately keeps its own one-flag `is_cancelled`.
   ...
   - Check permanent provider error (`is_api_error_banner` **and**
     ...
     the classification (`is_permanent`) so no "retrying" notice is emitted
...
- Logs and deletes any file named by `_RETIRED_DEFERRED_SUFFIXES` — a suffix the framework used to honour and no longer replays. Runs **first** in `_drain_deferred_ops` because it depends on nothing; its only hard constraint is that `_warn_unconsumed_deferred_files` follows it. Ordering used to carry a second job — a later handler raising stranded the file until the temp sweep — which the drain's per-handler guard took over (ISSUE-469).
...
- `_process_deferred_kv_ops` — `task_{id}_kv_ops.json` from `istota-skill kv set|delete|set-add|set-remove|set-trim`. The set ops re-read the current value before applying, so they compose across concurrent tasks; `set-trim` carries only `keep_newest` and is skipped (rather than creating the key) when the row is absent. Like the health and KG loops it now skips a non-dict entry — `_load_deferred_json` type-checks the outer list only, and `entry.get` is read before the per-op `try`, so a bare string in a model-written file raised past the guard and lost the rest of that file — and, before ISSUE-469, the rest of the drain (ISSUE-451). Same guard in `_process_deferred_sent_emails`.
- `_process_deferred_user_alerts` — `task_{id}_user_alerts.json` for the alerts/notification path.
- `_process_deferred_health_ops` — `task_{id}_health_ops.json` from `istota-skill health log|add-panel|add-biomarker|upload|set|...`. Resolves the user's `HealthContext` and replays insert/update ops against the per-user `health.db`. User id always comes from the task (defense-in-depth); recognized suffix lives in `_KNOWN_DEFERRED_SUFFIXES` so unknown-suffix warnings stay clean. Four of its ops touch the filesystem: `attach_document` (health-document-attachments spec) copies a user-supplied file into the health document store and links it to an encounter / diagnosis / immunization, `register_upload` copies one in under a new panel id, `import_csv` reads a CSV of bloodwork results, and `detach_document` removes one link. Because the op file is written *inside* the sandbox, `source_path` is attacker-influenced text while the replaying daemon is not sandboxed — `_resolved_source_path` therefore resolves symlinks, confines the result (`_source_path_allowed`) to the task's own deferred dir or the user's base workspace `{mount}/Users/{uid}`, and returns the approved path so the read uses *it* rather than re-resolving (which would reopen the swap window the check just closed). The base dir rather than the bot subdir, because the driving case is an email attachment the executor dropped in `inbox/`. `register_upload` now goes through the same guard — it had none, and the new op made the omission conspicuous. So does `import_csv` (ISSUE-447), which was the last of the three source-path arms reading its path unscoped: it took the string, called `is_file()` on it and read the bytes, so a task could file any host file the daemon can read into that user's health records. The three arms are now identical, and a fourth source-path op must join them. Both `attach_document` and the skill's direct path read the operator's `[health] max_document_bytes` (`_health_max_document_bytes`) instead of the library default, and validate that the target encounter / diagnosis / immunization exists before writing bytes (a link to a nonexistent row would be invisible *and* sweep-exempt). An `attach_document` may carry `encounter_ref` instead of `entity_id`, resolved against a separate `encounter_refs` dict populated by `insert_encounter`'s own `ref` (kept apart from the panel `refs` dict so a shared name can't cross-resolve); an unresolved ref raises rather than mis-filing paperwork, landing the op in `failures`. The op loop's except tuple gained `OSError` + `DocumentError` so one refused attachment (bad MIME, over cap) fails only its own op, and is now **`except Exception`, like its two neighbours** (ISSUE-451). Enumerating did not converge: every field is model-authored JSON and the body coerces it about twenty times across twenty-five op arms, so the tuple was missing `TypeError` for `Path(123)` and `float([1])`, and adding that still left `OverflowError` for `int(Infinity)` — which `json.loads` accepts by default. Anything escaping there costs every later op in the file — and used to cost every handler after it as well, until the drain started guarding each one (ISSUE-469). Nothing is lost by widening: the failing op is rolled back, logged at ERROR and written to the failure sidecar, which is better journaling than either neighbour has. `_load_deferred_json` keeps an enumerated catch (it reads one file and parses it) and gained `RecursionError` there, since deeply nested model-authored JSON raises it rather than a `JSONDecodeError`. That handler also now `conn.rollback()`s: the loop commits per op, so a failing op's partial work stayed open on the connection and the *next* op's commit swept it in — an `attach_document` that inserted its row and then failed to write the bytes would have silently persisted a row pointing at a file that never landed. Prior ops are already committed, so the rollback can only discard the failed one.
...
**Encoding**: every deferred-file read and write names `encoding="utf-8"` explicitly, and `_load_deferred_json` catches `UnicodeDecodeError` alongside `JSONDecodeError`/`OSError`. The producer is a task subprocess whose env was rebuilt from scratch (`build_stripped_env`) while the consumer is the daemon under systemd's, so nothing guarantees the two agree on `locale.getencoding()`. The catch used to be the load-bearing half — a decode error escaping the first handler silently skipped the eight after it — and since ISSUE-469 it decides only how the failure reads: a warning naming the file, rather than an ERROR with a stack for a locale mismatch.

**The drain is guarded per handler (ISSUE-469)**, so no `Exception` escapes `_drain_deferred_ops` and one bad file costs its own handler and nothing else. Five things about the shape:

- **Per handler, not one `try` around the sequence.** Whole-sequence containment is the smaller diff and still skips the eight handlers after the failure; it would only have protected the delivery that follows. Per-handler is what the individual guards were each reaching for one enumerated exception tuple at a time — ISSUE-135 and ISSUE-451 were both closed by hardening a single handler, and ISSUE-451's own resolution recorded the sequence as the larger half it was leaving.
- **The table is built inside the function**, not at import, so the handler names resolve off the module at call time — which is both what lets a test patch one and what stops the table from freezing a stale reference.
- **The task stays `completed`.** The row is written before the drain runs and everything reading it has already been told the task succeeded, so failing it here would put a task that answered correctly back on the retry ladder and re-execute it. `_record_deferred_handler_failure` is what stands in: an ERROR **with `exc_info`** on the daemon log, plus a `task_logs` row so the failure is visible to anything reading the task. Previously the only trace was `UserWorker.run`'s own catch — one line, no stack, for a drain that had been silently cut short.
- **Delivery stays where it is**, after the drain, rather than being hoisted ahead of it. The reorder was the other candidate fix and is unnecessary once the drain cannot raise; hoisting would have changed a path where the drain runs first by design.
- **A `BaseException` is recorded and then re-raised.** `except Exception` alone left one real hole: `asyncio.CancelledError` is a `BaseException`, and two handlers notify through `run_coro` (`_process_deferred_garmin_import`'s success notice, and `_process_deferred_user_alerts`'s unrecorded fallback), so a worker still draining when `AsyncRuntime.stop` cancels in-flight coroutines takes one — skipping delivery for a row already marked `completed`, which is the ISSUE-469 failure exactly. Swallowing it would keep the drain running against a cancelled runtime, which is worse than losing a delivery the shutdown was going to lose anyway, so it propagates; the `task_logs` breadcrumb is written first. `KeyboardInterrupt` and `SystemExit` take the same arm, which is right on the `run_task_inline` / REPL path; in the daemon `_signal_handler` only sets `_shutdown_requested`, so neither is raised into this path.

The recorder opens its connection at **`main_loop_read_timeout_ms`, not `get_db`'s 30s default**. It runs once per failing handler, so at the default a framework DB locked by something long — the sleep cycle holds one write transaction per half, across LLM calls — would stall the worker 30s per handler between the `completed` row and the delivery this guard exists to protect. The `task_logs` message carries the exception *type* and not its message: these exceptions can carry model-authored file contents and host paths, and `task_logs` is a cross-user admin surface. The type separates a locked DB from a bad op file, and the stack is in the journal.

A handler's own guard is still load-bearing for the half the drain's cannot reach: the ops after the failing one **inside the file it is halfway through**. `_process_deferred_user_alerts`'s guard is a third case — its except *recovers*, sending the alert directly with no row, so letting the exception reach the drain would contain it and lose the alert.
...
| DB backup | `db_backup.backup_databases()` (SQLite online-backup → `{mount}/Backups/db/snapshots/<date>`; retention prune + collapse guard + `_alert_backup_problems` + staleness alert) | `db_backup_interval` | — (logs + operator alerts) |
...
| `db_backup_enabled` / `db_backup_interval` / `db_backup_dir` / `db_backup_retention` | true / 86400s / "" / 7 | `db_backup.backup_databases(config, today=None)` snapshots the framework DB + every per-user module DB to `db_backup_dir/<YYYY-MM-DD>/…` (default `{nextcloud_mount}/Backups/db/snapshots`) via SQLite's online-backup API — off-host durability now that module DBs left the Nextcloud-synced workspaces. **Dated dirs, not a single overwritten slot** (ISSUE-159): a corrupted/emptied live DB can't clobber the last good copy. `db_backup_retention` keeps the N newest dated dirs (0 = keep all) but never prunes a dir holding the newest *good* copy of any DB (`_prune_old_snapshots` protects it). A **collapse guard** (`_apply_collapse_guard` → `db_relocate._data_row_count`) quarantines a fresh snapshot as `*.suspect` (status `suspect`) when a DB that previously held data comes back empty/unreadable; exact-zero only (framework `tasks` legitimately shrinks under retention cleanup). Backup tree is `0700`/files `0600`. Cold copies are forced to DELETE journal mode (a WAL header would SIGBUS on the FUSE mount if ever opened in place). **Mount-liveness guard** (`_destination_is_durable`): a destination at or under `nextcloud_mount_path` is written only when `os.path.ismount` is true — a down rclone FUSE mount reverts to a local dir, and a naive `mkdir` would silently write the "backup" to local disk under the stale mountpoint; the run is skipped instead and the clock left stale. **Keyed on where the path resolves, not on which config branch produced it** (ISSUE-480): the derived default and an explicit `db_backup_dir` under the mount carry the same exposure and get the same check, where the explicit one used to be trusted unconditionally. Containment is `user_scope.is_within` over resolved paths, so a symlinked `db_backup_dir` landing inside the mount is caught and a mere textual prefix is not; the mount is resolved **once** and that one value feeds both the containment test and `ismount`, because `posixpath.ismount` short-circuits to False on a symlink and a split would report a symlinked mount path as permanently down. A destination outside the mount stays trusted — an operator naming `/mnt/backup-nas/…` asserts a durability this module cannot check. The whole check is gated on `storage_is_nextcloud` (`bool(nextcloud.url)`), the same predicate `doctor.check_mount_liveness` and `user_briefings` put in front of the same `ismount` call: without a Nextcloud, `nextcloud_mount_path` is a plain local folder that will never be a mountpoint, and a standalone install's `db_backup_dir` sits inside it by design. Consequence worth knowing: adding a `[nextcloud] url` to a config whose mount path is an ordinary directory stops backups. The clock is **persisted** to `{db_path.parent}/.db_backup_last_run` and seeded at boot via `db_backup.last_backup_time`, so it survives restarts. It advances **only when ≥1 DB snapshotted OK** — a fully-errored (or mount-down) run leaves it stale so the staleness alert can fire. The scheduler alerts the operator on any errored/suspect DB (`_alert_backup_problems`) and on **staleness** (`_maybe_alert_backup_stale`: persisted last-run older than `2 × db_backup_interval`; re-armable, gated on a prior successful run so a fresh deploy doesn't false-alarm). Both go through `_send_operator_alert`, which runs `send_notification` on a short-lived daemon thread with a join timeout so a wedged Talk can't stall the dispatch loop (ISSUE-143 class). Force an immediate backup with `python -m istota.db_backup` (ignores the interval; closes the first-run gap after a deploy). Restore via `db_restore` (`python -m istota.db_restore --all`) — copies the newest good cold copy back (or `--date`), clears stale `-wal`/`-shm` sidecars, refuses an empty snapshot without `--force`, and refuses to run while the daemon holds its flock (`_daemon_running`) since a copy over a live WAL DB corrupts it; then `init_db` re-flips WAL |
...
| `_run_capture()` | Subprocess runner for both `_execute_command_task` and `_execute_skill_task`. Runs the child under `subprocess.Popen(start_new_session=True)` and, on `TimeoutExpired`, SIGKILLs the whole process group via the shared `process_group.kill_group_if_live` (then a bounded second `communicate`) before re-raising. The local `_kill_process_group` wrapper is gone (F5): it was the one of three copies with no reaped-process guard, and the shared helper carries it — signalling a pid the OS has already handed to somebody else would take that process's whole group. `brain/claude_code`'s `_kill_group_if_live` closure and `session/tools/bash._kill_process_group` are the other two, both now the same call. `subprocess.run(timeout=…)` only SIGKILLs the *direct* child and then blocks forever in its post-kill `communicate()` when an orphaned grandchild still holds the stdout/stderr pipe — a CRON `command:` that backgrounds a child (or a skill CLI that shells out) wedged its worker past the timeout that way, and since the per-task heartbeat thread keeps pinging, the liveness-based stuck-running reaper never reclaimed it (a per-minute job held its only background slot 6.5h). Returns a `CompletedProcess` so callers keep using `.returncode`/`.stdout`/`.stderr`; re-raises `TimeoutExpired` so their existing except branches are unchanged. |
