...
`execute_task` creates `<unit cgroup>/task-<id>/` with `memory.max`, `pids.max` and `cpu.max` before the brain runs, and hangs `destroy` off the same `ExitStack` that owns the skill and network proxies, so success, failure, timeout, cancellation and a fallback brain replacing the primary all give the directory back. `run_daemon` sweeps `task-*` directories left by a previous run — a daemon killed by the OOM killer does not run its own cleanup, which is precisely the event here.

- **The directory is a sibling of the daemon's own cgroup, not a child of it.** cgroup v2 forbids a non-root cgroup from both holding processes and enabling controllers for its children (`EBUSY` on the `cgroup.subtree_control` write), so a `task-<id>/` made inside the cgroup the daemon sits in is created successfully and then contains no `memory.max` at all. `DelegateSubgroup=supervisor` puts the daemon in a leaf and leaves the *unit* cgroup free to enable controllers; `resolve_root` walks up from `/proc/self/cgroup` to the `.service`/`.scope` component to find it, the same walk `host_pressure.read_memory_events` makes for the same reason.
...
- **Two placement paths, because the brains have two shapes.** ClaudeCodeBrain and TmuxClaudeBrain spawn one long-lived child and report it through `on_pid`, which the executor uses to place it — membership is inherited across `fork`, so the whole tree goes with it. NativeBrain never calls `on_pid`: it spawns a fresh child per Bash execution. It gets the path on `BrainRequest.task_cgroup` → `ToolEnv.task_cgroup` and the Bash tool places each child itself. Without that second path the brain that would actually run a task's `pytest` is the one brain containment silently skips.
- **Placement is from the parent after spawn, not from a `preexec_fn`.** The daemon is heavily threaded and a `preexec_fn` runs between `fork` and `exec`, where opening a file is not async-signal-safe. The cost is a short window in which the child is outside the cgroup, which is negligible against a `bash -c` that has yet to exec its real work.
- **Fails open, never fails silent.** No `Delegate=`, an older systemd that ignored `DelegateSubgroup=`, cgroup v1, macOS: `create` returns `None`, the task spawns exactly as before, and the reason is logged once per process rather than once per task. The `STARTUP Per-task cgroups:` line reports which of those happened up front — without it, "containment on" and "containment silently absent" differ only by a warning that fires whenever the first task happens to run.
