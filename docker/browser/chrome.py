...
# browse-watchdog thread. Without it, the watchdog's recover_wedged_chrome(inst)
# nulls inst.proc and relaunches while a freshly-unblocked Flask request runs
# ensure_chrome(inst) concurrently -- two Popen()s race for the same --user-data-dir
...
# launch_chrome(inst) while already holding it. Deliberately does NOT guard the
...
# One lifecycle lock covers all instances: Flask serializes requests, and
# multiple locks would add ordering without allowing concurrent Patchright work.
_chrome_lock = threading.RLock()

# Process handles and diagnostic records live on BrowserInstance in pool.py.

# True while launch_chrome(inst) is bringing Chrome up: the process exists but its
...
#
# A relaunch invalidates the session table's page objects while leaving the
# session entries untouched. Every request carrying a pre-kill session id then
...
# CDP arm only observes connect_cdp(inst), and the successful connect after each
...
# records without drawing conclusions -- the same split as inst.cdp_health above.
...
# with Chrome still up and every health probe still green. disconnect_cdp(inst)
...
# recover_wedged_chrome(inst) exists) but nothing enforced it, and the resource
...
# The driver is shared: even an instance without a connection must use the
# thread that started it. Ownership ends only when pool.cleanup stops the driver.
...
# which is what makes the record readable from any thread when connect_cdp(inst) is
# not.
#
...
# does not: recover_wedged_chrome(inst) kills and relaunches the browser, which
...
#
# 1. Only a call the caller cares about. connect_cdp(inst) is reached from teardown
...
#    legitimate recovery: recover_wedged_chrome(inst) kills Chrome, the unwinding
#    request calls _close_session() -> get_context(inst) during the relaunch, and
...
# cdp_health(inst). cleanup(inst) reaches it while holding _chrome_lock, so the ordering
...
# -- _chrome_explains_failure(inst) runs before it is taken, because it makes an HTTP
# call.
_cdp_health_lock = threading.Lock()


def _record_cdp_success(inst):
    """Note that a CDP call worked. Safe from any thread."""
    with _cdp_health_lock:
        inst.cdp_health["last_success"] = time.monotonic()
        inst.cdp_health["consecutive_failures"] = 0
        inst.cdp_health["last_error"] = ""


def _chrome_explains_failure(inst):
    ...
    Called before the health lock is taken: devtools_responding(inst) is an HTTP
    call, and holding a lock the liveness thread wants across it is the shape
    this module exists to avoid.
    """
    if not is_chrome_running(inst) or inst.launching:
        return True
    return not devtools_responding(inst, timeout=2)


def _record_cdp_failure(inst, error):
    """Note that a CDP call could not be made. Safe from any thread.

    One call is one failure. connect_cdp(inst) retries internally and this is
    ...
    *requests* rather than a multiple of the retry setting.
    ...
    """
    explained = _chrome_explains_failure(inst)
    with _cdp_health_lock:
        inst.cdp_health["last_failure"] = time.monotonic()
        inst.cdp_health["last_error"] = str(error)[:500]
        if not explained:
            inst.cdp_health["consecutive_failures"] += 1
...
def cdp_health(inst):
    ...
    """
    with _cdp_health_lock:
        return dict(inst.cdp_health)


def launch_generation(inst):
    """How many times Chrome has been launched in this process. Any thread."""
    return inst.launch_generation


def record_wedge_recovery(inst):
    ...
    """
    with _wedge_lock:
        inst.wedge_recoveries.append(time.monotonic())
        if len(inst.wedge_recoveries) > WEDGE_HISTORY_MAX:
            del inst.wedge_recoveries[:-WEDGE_HISTORY_MAX]


def wedge_recovery_history(inst):
    """Copy of the recovery timestamps (time.monotonic). Safe from any thread."""
    with _wedge_lock:
        return list(inst.wedge_recoveries)
...
def launch_chrome(inst):
    ...
    args = [
        ...
        f"--remote-debugging-port={inst.cdp_port}",
        f"--disk-cache-size={os.environ.get('BROWSER_DISK_CACHE_BYTES', '104857600')}",
        "--no-first-run",
    ...
    with _chrome_lock:
        ...
        try:
            ...
            inst.launch_generation += 1
            _wait_for_chrome_ready(inst)
            log.info(
                "Chrome launched (pid=%d, debug_port=%d, generation=%d)",
                inst.proc.pid, inst.cdp_port, inst.launch_generation,
            )
        finally:
            inst.launching = False


def devtools_responding(inst, timeout=2):
    ...
    try:
        resp = urllib.request.urlopen(
            f"http://localhost:{inst.cdp_port}/json/version", timeout=timeout,
        )
        resp.close()
        return True
    except Exception:
...
def is_launching(inst):
    """Whether a launch is in progress (DevTools not yet expected to answer)."""
    return inst.launching


def _wait_for_chrome_ready(inst, timeout=15):
    """Wait for Chrome's debugging port to accept connections."""
    deadline = time.time() + timeout
    while time.time() < deadline:
        if devtools_responding(inst, timeout=2):
            return
        time.sleep(0.5)
    raise RuntimeError(
        f"Chrome not ready on port {inst.cdp_port} after {timeout}s",
    )


def ensure_chrome(inst):
    """Ensure Chrome process is running, relaunch if dead.

    Flask-thread-only: reaches disconnect_cdp(inst). A watchdog thread wanting Chrome
    back wants recover_wedged_chrome(inst) instead. See the _chrome_lock comment.
    """
    with _chrome_lock:
...
def restart_chrome(inst):
    ...
    Flask-thread-only: reaches disconnect_cdp(inst). See the _chrome_lock comment.
    """
    with _chrome_lock:
        disconnect_cdp(inst)
...
def recover_wedged_chrome(inst):
    """Kill and relaunch Chrome from a watchdog thread (Playwright-free).

    Like restart_chrome(inst), but it does NOT call disconnect_cdp(inst): Patchright's
    ...
    connection is rebuilt lazily by the next connect_cdp(inst) (which already
    ...
    with _chrome_lock:
        ...
        # here on. The driver still belongs to its original thread; restarting
        # one Chrome cannot move the shared asyncio loop to another thread.
    ...
    record_wedge_recovery(inst)


def is_chrome_running(inst):
    """Check if Chrome process is alive."""
    return inst.proc is not None and inst.proc.poll() is None


def _assert_pw_thread(inst, op, record=True):
    """Refuse a Patchright call from a thread that doesn't own the connection.

    The shared driver and each connection belong to their opening thread. Even
    a disconnected instance must use the shared driver's thread. Raises
    ...
    """
    owner = _driver_thread_id if _pw is not None else inst.pw_thread_id
    ...
    log.error(
        ...
        "thread, or use recover_wedged_chrome(inst) if you only need the OS process.",
        op, threading.current_thread().name, owner,
    )
    err = RuntimeError(
...
def connect_cdp(inst, retries=3, record=True):
    ...
    if inst.pw_browser is not None:
        ...
        try:
            session = inst.pw_browser.new_browser_cdp_session()
            try:
                session.detach()
            except Exception:
                pass
    ...
    for attempt in range(retries):
        try:
            ...
            # last leaves a window where inst.pw_context is live and the guard is
            # still inert, which is the one call the guard exists to refuse.
            # Failure clears this instance's connection. The shared driver
            # remains available to the other instances.
            inst.pw_thread_id = threading.get_ident()
            if _pw is None:
                _driver_thread_id = threading.get_ident()
                _pw = sync_playwright().start()
            inst.pw_browser = _pw.chromium.connect_over_cdp(
                f"http://localhost:{inst.cdp_port}",
            )
    ...
    if record and _record_cdp_failure(inst, err):
        log.error(
...
def disconnect_cdp(inst):
    """Disconnect Patchright from Chrome (Chrome keeps running).

    Guarded like the rest: Browser.close() drives thread-bound machinery, so
    tearing the connection down from a foreign thread wedges
    the loop exactly as using it would. A watchdog that needs Chrome gone
    without touching Patchright calls recover_wedged_chrome(inst) instead.
    """
    # record=False: this is teardown. It is reached from connect_cdp(inst)'s own
...
def is_cdp_connected(inst):
    ...
    """
    return inst.pw_browser is not None


def get_context(inst, record=True):
    """Get the Patchright browser context (connects if needed).

    ``record`` passes straight through to connect_cdp(inst); see the note there.
    """
    connect_cdp(inst, record=record)
    return inst.pw_context


def get_page_by_index(inst, tab_index):
    """Get a page by tab index from the connected context."""
    _assert_pw_thread(inst, "get_page_by_index")
    if not inst.pw_context:
        return None
    pages = inst.pw_context.pages
...
def cleanup(inst):
    ...
    Flask-thread-only: reaches disconnect_cdp(inst). Registered via atexit, which
