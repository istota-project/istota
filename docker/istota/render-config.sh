...
# missing for the session secret, ENOSPC, a future unguarded `${VAR}` read.
# entrypoint.sh then sees a file at `$CONFIG_FILE` on the next boot, skips the
# render for good, runs the backfill passes over the fragment and execs the
# daemon on it. That is the one failure mode here that turns into a silent
# production incident rather than a loud boot failure.
#
