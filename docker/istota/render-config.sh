...
ISTOTA_SMS_TWILIO_ACCOUNT_SID="$(toml_escape "${ISTOTA_SMS_TWILIO_ACCOUNT_SID:-}")"
ISTOTA_SMS_TWILIO_AUTH_TOKEN="$(toml_escape "${ISTOTA_SMS_TWILIO_AUTH_TOKEN:-}")"
ISTOTA_SMS_TWILIO_API_KEY_SID="$(toml_escape "${ISTOTA_SMS_TWILIO_API_KEY_SID:-}")"
ISTOTA_SMS_TWILIO_API_KEY_SECRET="$(toml_escape "${ISTOTA_SMS_TWILIO_API_KEY_SECRET:-}")"
ISTOTA_SMS_TWILIO_MESSAGING_SERVICE_SID="$(toml_escape "${ISTOTA_SMS_TWILIO_MESSAGING_SERVICE_SID:-}")"
ISTOTA_SMS_TELNYX_API_KEY="$(toml_escape "${ISTOTA_SMS_TELNYX_API_KEY:-}")"
ISTOTA_SMS_TELNYX_PUBLIC_KEY="$(toml_escape "${ISTOTA_SMS_TELNYX_PUBLIC_KEY:-}")"
...
TOML
    ...
    SMS_SERVICE_NUMBERS="$(toml_string_list "${ISTOTA_SMS_SERVICE_NUMBERS:-}")"
    [ -n "$SMS_SERVICE_NUMBERS" ] || SMS_SERVICE_NUMBERS="[]"
    cat >> "$CONFIG_FILE" <<TOML

[sms]
enabled = ${ISTOTA_SMS_ENABLED:-false}
provider = "${ISTOTA_SMS_PROVIDER}"
service_numbers = ${SMS_SERVICE_NUMBERS}
default_sender_number = "${ISTOTA_SMS_DEFAULT_SENDER_NUMBER}"
max_segments = ${ISTOTA_SMS_MAX_SEGMENTS:-6}
request_timeout_seconds = ${ISTOTA_SMS_REQUEST_TIMEOUT_SECONDS:-10}

[sms.twilio]
account_sid = "${ISTOTA_SMS_TWILIO_ACCOUNT_SID}"
auth_token = "${ISTOTA_SMS_TWILIO_AUTH_TOKEN}"
api_key_sid = "${ISTOTA_SMS_TWILIO_API_KEY_SID}"
api_key_secret = "${ISTOTA_SMS_TWILIO_API_KEY_SECRET}"
messaging_service_sid = "${ISTOTA_SMS_TWILIO_MESSAGING_SERVICE_SID}"
...
TOML
    ...
    # KDBX credential vault (ISSUE-505). These two have no user_profiles column
    # — neither the CLI nor the web UI may set them — so this file is the whole
    # surface, and it is rewritten on every boot. The passphrase is not here: it
    # is an encrypted secret, provisioned with `istota secret ensure --service
    # vault --key passphrase --generate`. An ineligible service name is rendered
    # as written and dropped with a warning at load, which is where an operator
    # can act on it.
    if [ -n "${USER_VAULT_PATH:-}" ]; then
        echo "vault_path = \"$(toml_escape "${USER_VAULT_PATH}")\"" >> "$CONFIG_FILE"
    fi
    _vault_services_list="$(toml_string_list "${USER_VAULT_SERVICES:-}")"
    if [ -n "$_vault_services_list" ]; then
        echo "vault_services = $_vault_services_list" >> "$CONFIG_FILE"
    fi
