...
`health` skill (`src/istota/skills/health/`): `log|stats|latest|panels|panel|add-panel|add-biomarker|trend|upload|import-csv|export-csv|summary|settings|set`. `setup_env` hook injects `HEALTH_DB_PATH`. Writes flow through the deferred-op file `task_<id>_health_ops.json` under sandbox; `_process_deferred_health_ops` replays them post-task (including the `import_csv` op, which re-reads the source file from the deferred path on the host side). Skill self-gates on `module_health` and returns a JSON error envelope when the flag is off.
