...
- **No databases in the sandbox**: `build_bwrap_cmd` ends by masking `db_path.parent` and `module_db_root()` with an empty tmpfs — the last mount operations, so no earlier bind shows through (`--disable-userns` too, where bwrap supports it — a tmpfs can otherwise be unmounted from a nested user namespace, so keep `sandbox_ro_paths` narrow rather than relying on the mask to make a broad entry safe). Nothing binds the framework DB for anyone (`sandbox_admin_db_write` is retired), `sandbox_ro_paths` defaults to `[]`, and `native_fs_roots` matches. Reads and writes go through skill CLIs that run host-side and scope by `ISTOTA_USER_ID` — that scoping is the per-user boundary; the masks are defence in depth behind it.
