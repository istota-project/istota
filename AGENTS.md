...
Role at `deploy/ansible/` (symlinked from `ansible-server/roles/istota/`). When adding config fields, update `defaults/main.yml` and `templates/config.toml.j2`.
