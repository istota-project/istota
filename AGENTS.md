...
├── task_cgroup.py        # A cgroup v2 group per task (A6): `<unit cgroup>/task-<id>/` with `memory.max`, `pids.max` and `cpu.max`, so a runaway process tree is OOM-killed inside its own cgroup instead of taking the host. The directory is a *sibling* of the daemon's `supervisor/` leaf, not a child of it — cgroup v2 refuses to let a cgroup both hold processes and enable controllers for its children, so a group made inside the daemon's own would silently contain no `memory.max` at all; `resolve_root` walks up to the `.service` component the same way `host_pressure.read_memory_events` does. The kernel is the capability probe: interface files cannot be created by a writer, so a successful `memory.max` write *is* the proof the controller is delegated. Fails open on any deployment without `Delegate=` and says so once at startup rather than once per task. stdlib-only leaf, roots are parameters, never raises
