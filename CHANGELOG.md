...
- Location: phantom-stop guard. A cluster only inherits a place_id when its centroid falls inside the place's radius — previously a single grazing ping during a drive-by could promote transit through that place into a phantom stop because place-matched clusters bypass min-pings / min-dwell filters.
