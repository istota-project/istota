...
### Fixed
- Web chat no longer drops detail when the assistant works in steps. When it wrote a substantial explanation, then took an action (like editing a note), then gave a short confirmation, the explanation used to vanish from the reply once the action started — leaving only the terse wrap-up. Substantial intermediate passages now stay visible as their own blocks in the answer, in order, with the tool activity folded between them. Short lead-ins ("Let me check…") are still hidden. Applies to the live web/REPL view and to reloaded history alike.
