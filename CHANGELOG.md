...
- Log channel finalize line shows the resolved model + effort (`(model: claude-opus-4-7 high)`) on every task entry, so per-job overrides are visible at a glance without cross-referencing CRON.md.
