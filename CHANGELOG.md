...
- Any block of a chat message can be copied. Hovering a paragraph of a reply — or tapping it on a phone — reveals a copy button at its bottom-right that puts that block on the clipboard as the original Markdown, so pasting it elsewhere gives back what was written rather than a wall of formatting. A reply that ran tools is several blocks, and each is copied on its own; the tool traces between them are never included. Code keeps its fences, so a block containing a snippet copies ready to paste. Your own messages and command output copy the same way.
