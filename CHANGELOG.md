...
- A currency figure is shown only where it is real money. On a subscription the number the tool reports is what the same usage would have cost at list price, and on some setups it is a catalog estimate that prices an unknown model at zero — both read as spend at a glance. Those show a dash and the reason instead, and are never added to a real figure; usage is read through its token counts there. Tasks that report nothing at all are counted and named rather than averaged in as zero.
