...
### Fixed
- Status labels in the web UI read correctly in light theme. Several coloured chips and badges — the effort tag beside the configured model on the admin page, the "critical" flag on a bloodwork value — were painted with dark-theme colours regardless of the theme, landing as heavy dark blocks on a white page. Colours for the four status meanings (error, warning, success, information) are now defined once for each theme and used everywhere, so the same state looks the same wherever it appears and picks the right shade for the theme you're in. Category colours that distinguish *kinds* of thing rather than severity — encounter types, task sources — deliberately keep their own distinct hues.
...
### Fixed
- **Runtime memory writes can no longer land under the wrong heading.** The previous skill recipe was `echo "- … (noted $(date +%Y-%m-%d))" >> USER.md`, which appends to EOF — i.e. under whatever `## ` heading is last in the file. Real-world failure: temporal facts about a fountain-pen purchase / decision / return ended up under "Emailing on alice's behalf" because that section happened to be last. The runtime CLI requires `--heading` and rejects unknown ones with the available-heading list, so the model self-corrects instead of dumping under the wrong section.
