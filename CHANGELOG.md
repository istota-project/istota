...
### Fixed
- Connecting Monarch Money with your email and password works again. Monarch had started rejecting the app version we identify ourselves as, and because it only checks that *after* your credentials are accepted, the symptom was a "service unavailable" error on a correct email and password — it read as an outage rather than something on our side. We now look up the version Monarch's own web app reports before connecting, so a version we have never seen fixes itself rather than failing until an update ships; it turns out to change within hours, so a fixed value was never going to hold.
