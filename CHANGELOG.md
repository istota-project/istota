...
### Security
- Outbound email recipient gate (Layer A of the adversarial-defense plan). Sends to addresses not in the user's known-recipients set are held for explicit user confirmation instead of going out immediately. The known-set is built from prior correspondence (sent + received), the runtime trusted-senders table, and the user's own configured addresses. For trusted task sources (CLI, Talk, scheduled jobs, subtasks) addresses the user typed verbatim in the prompt are also allowed; email-sourced tasks deliberately do not get this auto-allowlist so an inbound email body cannot smuggle its own attacker address. Confirmation is posted in the user's main conversation in the same `"I need your confirmation to proceed: ..."` natural-language format used for other sensitive-action confirmations (file deletion, calendar deletion, external sharing). Reply `yes` to send, `yes trust` to also add the address to the trusted-recipients table for future tasks, `no` to discard. The `sensitive_actions` skill no longer asks the agent to pre-confirm email sends — the system gate handles it deterministically. New `[security] outbound_gate_email = true` (default on) toggles the gate; new Ansible var `istota_security_outbound_gate_email`.
- Browse container now uses Quad9 DNS (`9.9.9.9` / `149.112.112.112`) for security-only filtering of known-malicious destinations. Defense in depth — does not replace any other layer and cannot stop attacker-controlled fresh domains, but blocks known-bad destinations and compromised legit pages serving malware.
