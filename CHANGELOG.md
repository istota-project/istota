...
### Security
- Outbound email recipient gate (Layer A of the adversarial-defense plan). Sends to addresses not in the user's known-recipients set are held for explicit user confirmation via the alerts channel instead of going out immediately. The known-set is built from prior correspondence (sent + received), the runtime trusted-senders table, and the user's own configured addresses. For trusted task sources (CLI, Talk, scheduled jobs, subtasks) addresses the user typed verbatim in the prompt are also allowed; email-sourced tasks deliberately do not get this auto-allowlist so an inbound email body cannot smuggle its own attacker address. Reply "yes" to send, "yes trust" to also add the address to the trusted-recipients table for future tasks, "no" to discard. New `[security] outbound_gate_email = true` (default on) toggles the gate; new Ansible var `istota_security_outbound_gate_email`.
