...
### Fixed
- A briefing block reading a todo file can be told to cover only one part of it. The file's headings used to be stripped before the briefing was written, so every item arrived as one undifferentiated list and an instruction like "only show items under NOW" was impossible to follow rather than merely ignored. Items now reach the briefing grouped under the heading they sit beneath.
