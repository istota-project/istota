...
- The blanked-out database directories inside the sandbox are now read-only as well as empty. Opening a database path there used to create an empty file and answer "no such table", which reads as a missing schema rather than as a boundary and sends a task hunting for a database that was never in reach — and left the stray file sitting there for the rest of the run. The same attempt now fails outright, and nothing a task writes under those directories can be mistaken for a database later.
