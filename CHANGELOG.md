...
- Deleting mail no longer reaches past the messages Istota picked. Both the retention sweep and the delete command marked their messages and then asked the server to clear the folder, which also permanently removes anything another mail client has marked for deletion and not yet cleared. They now remove exactly the messages they chose. A server too old to support that gets the old behaviour and one warning saying so.
