...
- Warnings that used to be sent once and then vanish now leave an entry in the bell too. A security concern the bot raised while reading a message, a request you emailed in that timed out before you answered it, an answer that could not be delivered, and a warning that mail claiming to come from your own address failed its authentication check were each a single push and nothing behind it — so if you had no alert destination set up, or the send simply failed, the warning was gone and in one case the evidence was deleted with it. Each now leaves an entry carrying the detail, and clears itself once you have read it. Repeats collapse rather than pile up: fifty forged messages in one burst are one entry counted fifty times, not fifty warnings. Entries nobody reads are cleared after two weeks, so the count cannot climb forever.
